// @ts-nocheck
// ============================================
// Shahin GRC — Audit Advanced Service
 ...
 */
export async function getAuditUniverse(tenantId: string): Promise<any[]> {
...
export async function generateCommitteeReport(
  ...
  const dateFilterPlans = startDate && endDate
    ? `AND rp.created_at >= '${startDate}' AND rp.created_at < '${endDate}'`
    : "";
...
export async function getFindingAging(tenantId: string): Promise<FindingAging[]> {
  ...
  return result.rows.map((row: GenericRow) => {
    const slaDays = SLA_THRESHOLDS[String(row.severity)] || SLA_THRESHOLDS.medium;
...
export async function createQAReview(
  ...
  const s = tenantSchema(tenantId);
  const validRatings = ["satisfactory", "needs_improvement", "unsatisfactory"];
  if (data.overall_rating && !validRatings.includes(data.overall_rating)) {
    throw new Error(
      `Invalid overall_rating. Must be: ${validRatings.join(", ")}`
    );
  }
...
export async function shareWithExternalAuditor(
  ...
  // Merge new IDs with any previously shared IDs (deduplicated)
  const result = await safeQuery(
    `UPDATE "${s}".external_audit_requests
     SET
       shared_findings = (
     ...
     WHERE request_id = $1
     RETURNING *`,
    [requestId, JSON.stringify(findingIds), JSON.stringify(evidenceIds)]
  );

  if (!result.rows[0]) {
    throw new Error("External audit request not found");
  }
  return result.rows[0];
}
