...
export async function getScoresForEntity(tenantId: string, universeId: string): Promise<any> {
...
export async function computeWeightedScore(tenantId: string, universeId: string): Promise<any> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT
...
export async function getRankedListWithRiskRegister(tenantId: string): Promise<any> {
  const s = tenantSchema(tenantId);
  try {
    const result = await safeQuery(`
      SELECT
...
export async function getRiskRankedList(tenantId: string): Promise<any> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT
