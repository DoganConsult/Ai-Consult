...
export async function listTestPlans(tenantId: string, auditId: string): Promise<any> {
...
export async function updateTestResult(
  ...
  const s = tenantSchema(tenantId);
  const validStatuses = ['pending', 'in_progress', 'passed', 'failed', 'not_applicable'];
  if (!validStatuses.includes(status)) {
    throw new Error(`Invalid status. Must be: ${validStatuses.join(', ')}`);
  }
  const result = await safeQuery(
    `UPDATE "${s}".audit_test_plans
     SET status = $2, result_notes = $3, tested_by = $4,
         tested_at = NOW(), updated_at = NOW()
     WHERE plan_id = $1 AND deleted_at IS NULL RETURNING *`,
    [id, status, resultNotes, testedBy]
  );
  if (!getFirstRow(result)) throw new Error("Test plan not found");
  return getFirstRow(result);
}

// ── Get control coverage for an audit ────────────────────────────────

export async function getControlCoverage(tenantId: string, auditId: string): Promise<any> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT
