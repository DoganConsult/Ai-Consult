...
export async function listRepeatFindings(tenantId: string): Promise<any> {
...
export async function getRepeatHistory(tenantId: string, findingId: string): Promise<any> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT rf.*,
