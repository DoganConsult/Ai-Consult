...
export async function generateExecutiveSummary(tenantId: string): Promise<any> {
...
export async function getCommitteeMetrics(tenantId: string): Promise<any> {
  const s = tenantSchema(tenantId);
...
export async function getBoardDashboard(tenantId: string): Promise<any> {
  const s = tenantSchema(tenantId);

  const safeCount = (q: string, params?: any[]) =>
