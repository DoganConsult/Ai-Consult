...
import { recordObservation } from '../../ai/services/ai-observation.service';
...
export async function generateWorkpapers(
  ...
  const __evidenceMap = new Map(evidence.map((e) => [e.evidenceId, e]));
...
export async function generateBatchWorkpapers(
  ...
  for (const frameworkId of options.frameworkIds) {
    try {
      ...
      console.warn(`[WorkpaperGenerator] Failed to generate workpaper for framework ${frameworkId}: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  return workpapers;
}
