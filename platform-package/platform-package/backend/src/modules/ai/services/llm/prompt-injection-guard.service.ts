...
export interface InjectionDetectionResult {
  safe: boolean;
  detections: { type: string; severity: string; match: string }[];
  sanitized: string;
}

const INJECTION_PATTERNS: { name: string; regex: RegExp; severity: string }[] = [
...
export function detectInjection(input: string): InjectionDetectionResult {
  const detections: { type: string; severity: string; match: string }[] = [];
  let sanitized = input;
...
export async function logInjectionAttempt(
  ...
  try {
    await safeQuery(
      `INSERT INTO "${schema}".prompt_injection_log
...
export async function guardInput(
  ...
  userId?: string,
  agentId?: string,
): Promise<{ allowed: boolean; sanitizedInput: string; warning?: string }> {
...
export async function getInjectionStats(
  tenantId: string,
  daysBack: number = 30,
): Promise<{ total: number; blocked: number; byType: Record<string, number> }> {
  const schema = tenantSchema(tenantId);
  const stats = { total: 0, blocked: 0, byType: {} as Record<string, number> };

  try {
    const result = await safeQuery(
      `SELECT detection_type, blocked, COUNT(*)::int AS cnt
    ...
    for (const row of result.rows) {
      stats.total += row.cnt;
      if (row.blocked) stats.blocked += row.cnt;
      const types = (row.detection_type || '').split(',');
      for (const t of types) {
        if (t) stats.byType[t] = (stats.byType[t] || 0) + row.cnt;
      }
    }
  } catch { /* non-fatal */ }
