...
export async function evaluatePolicies(
  tenantId: string,
  context: { scope: string; targetId?: string; agentId?: string; actionType?: string; metadata?: Record<string, any> },
  ...
  try {
    const result = await safeQuery(
      `SELECT * FROM "${schema}".ai_policy_rule
       WHERE tenant_id = $1 AND enabled = TRUE
         AND (target_scope = 'all' OR target_scope = $2)
         AND (target_id IS NULL OR target_id = $3)
       ORDER BY severity DESC`,
      [tenantId, context.scope, context.targetId || ''],
    );

    for (const rule of result.rows) {
      if (!matchesCondition(rule.condition_json, context)) continue;
...
function matchesCondition(condition: Record<string, any>, context: Record<string, any>): boolean {
  ...
  for (const [key, expected] of Object.entries(condition)) {
    const actual = (context as any)[key] ?? context.metadata?.[key];
    if (actual !== expected) return false;
  }
  return true;
}

export async function listPolicyRules(tenantId: string, ruleType?: string): Promise<PolicyRule[]> {
  const schema = tenantSchema(tenantId);
  const conditions = ['tenant_id = $1'];
  const params: unknown[] = [tenantId];
  if (ruleType) { conditions.push('rule_type = $2'); params.push(ruleType); }

  const result = await safeQuery(
    `SELECT * FROM "${schema}".ai_policy_rule WHERE ${conditions.join(' AND ')} ORDER BY severity DESC, rule_name`,
    params,
  );
  return result.rows;
}
...
export async function createPolicyRule(
  tenantId: string,
  input: { ruleName: string; ruleType: string; targetScope?: string; targetId?: string; conditionJson?: Record<string, any>; actionOnMatch?: string; severity?: string; createdBy?: string },
): Promise<PolicyRule | null> {
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `INSERT INTO "${schema}".ai_policy_rule
       (tenant_id, rule_name, rule_type, target_scope, target_id, condition_json, action_on_match, severity, created_by)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
     RETURNING *`,
    [tenantId, input.ruleName, input.ruleType, input.targetScope || 'all', input.targetId || null, JSON.stringify(input.conditionJson || {}), input.actionOnMatch || 'block', input.severity || 'medium', input.createdBy || SYSTEM_JOB_ACTOR],
  );
  return getFirstRow(result) || null;
}

export async function deletePolicyRule(tenantId: string, ruleId: string): Promise<boolean> {
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `DELETE FROM "${schema}".ai_policy_rule WHERE rule_id = $1 AND tenant_id = $2 RETURNING rule_id`,
    [ruleId, tenantId],
  );
  return (result.rows?.length || 0) > 0;
}

export async function togglePolicyRule(tenantId: string, ruleId: string, enabled: boolean): Promise<boolean> {
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `UPDATE "${schema}".ai_policy_rule SET enabled = $3, updated_at = NOW() WHERE rule_id = $1 AND tenant_id = $2 RETURNING rule_id`,
...
export async function updatePolicyRule(
  tenantId: string,
  ruleId: string,
  updates: { ruleName?: string; ruleType?: string; targetScope?: string; targetId?: string; conditionJson?: Record<string, any>; actionOnMatch?: string; severity?: string },
): Promise<PolicyRule | null> {
  const schema = tenantSchema(tenantId);
  ...
  if (updates.ruleType !== undefined) { setClauses.push(`rule_type = $${idx++}`); params.push(updates.ruleType); }
  if (updates.targetScope !== undefined) { setClauses.push(`target_scope = $${idx++}`); params.push(updates.targetScope); }
  if (updates.targetId !== undefined) { setClauses.push(`target_id = $${idx++}`); params.push(updates.targetId); }
  if (updates.conditionJson !== undefined) { setClauses.push(`condition_json = $${idx++}`); params.push(JSON.stringify(updates.conditionJson)); }
  if (updates.actionOnMatch !== undefined) { setClauses.push(`action_on_match = $${idx++}`); params.push(updates.actionOnMatch); }
  if (updates.severity !== undefined) { setClauses.push(`severity = $${idx++}`); params.push(updates.severity); }
  if (setClauses.length === 0) return null;
  setClauses.push('updated_at = NOW()');
  params.push(ruleId, tenantId);
  const result = await safeQuery(
    `UPDATE "${schema}".ai_policy_rule SET ${setClauses.join(', ')} WHERE rule_id = $${idx++} AND tenant_id = $${idx} RETURNING *`,
    params,
  );
  return getFirstRow(result) || null;
}
...
function enforceThrottle(ruleId: string, windowMs: number, maxCount: number): boolean {
  const key = ruleId;
  const now = Date.now();
...
export function evaluateRateLimitPolicy(rule: PolicyRule, _context: Record<string, any>): { allowed: boolean; reason: string } {
  const windowMs = (rule.condition_json as any)?.windowMs ?? 60000;
  const maxCount = (rule.condition_json as any)?.maxCount ?? 10;
...
export async function detectPolicyConflicts(tenantId: string): Promise<Array<{ ruleA: string; ruleB: string; conflict: string }>> {
  const rules = await listPolicyRules(tenantId);
  const conflicts: Array<{ ruleA: string; ruleB: string; conflict: string }> = [];
...
export async function getBlockedActionLog(tenantId: string, hours: number = 24): Promise<any[]> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
...
export async function getPolicyEvalStats(tenantId: string): Promise<{ totalRules: number; enabledRules: number; blockRules: number; warnRules: number }> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
      `SELECT
