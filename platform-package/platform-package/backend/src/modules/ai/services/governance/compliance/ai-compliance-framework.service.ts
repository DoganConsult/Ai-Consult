...
export interface ComplianceFrameworkMapping {
  ...
  control_code: string;
  compliance_status: 'compliant' | 'partial' | 'non_compliant' | 'not_applicable' | 'not_assessed';
  evidence_ids: string[];
  last_assessed_at?: string;
  next_assessment_due?: string;
}

export interface FrameworkRiskClassification {
  classification_id: string;
  system_id: string;
  framework_code: string;
  risk_category: string;
  classification_answers: unknown;
  requirements_applicable: any[];
}

/**
 * Map AI system to compliance framework control
 */
export async function mapSystemToFramework(
  tenantId: string,
  mapping: {
    system_id: string;
  ...
  const result = await safeQuery(
    `INSERT INTO "${schema}".ai_compliance_framework_mapping 
...
export async function updateComplianceStatus(
  ...
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `UPDATE "${schema}".ai_compliance_framework_mapping 
 ...
 */
export async function classifyFrameworkRisk(
  tenantId: string,
  classification: {
    system_id: string;
    model_version_id?: string;
...
): Promise<FrameworkRiskClassification> {
  const schema = tenantSchema(tenantId);
  ...
  const result = await safeQuery(
    `INSERT INTO "${schema}".ai_framework_risk_classifications 
 ...
 */
export async function getSystemsRequiringAssessment(
  tenantId: string,
  frameworkCode?: string
): Promise<any[]> {
  const schema = tenantSchema(tenantId);
