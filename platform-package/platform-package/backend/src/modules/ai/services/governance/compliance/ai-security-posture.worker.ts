...
export interface SecurityPostureScore {
  ...
  moduleScores: ModuleSecurityScore[];
...
export interface ModuleSecurityScore {
  moduleCode: string;
  hasRoles: boolean;
...
export async function assessSecurityPosture(tenantId: string): Promise<SecurityPostureScore> {
  const schema = tenantSchema(tenantId);
  const dimensions: PostureDimension[] = [];
  const moduleScores: ModuleSecurityScore[] = [];
...
export async function getLastSecurityPosture(tenantId: string): Promise<SecurityPostureScore | null> {
  try {
    const schema = tenantSchema(tenantId);
    ...
    if (rows.length > 0) {
      ...
      return data as SecurityPostureScore;
    }
  } catch (err) {
...
async function assessRbacCoverage(schema: string): Promise<PostureDimension> {
...
async function assessSoDCoverage(schema: string): Promise<PostureDimension> {
  const items: PostureCheckItem[] = [];
  let score = 0;
  const maxScore = 20;
...
async function assessAuditCoverage(schema: string): Promise<PostureDimension> {
  const items: PostureCheckItem[] = [];
  let score = 0;
  const maxScore = 20;
...
async function assessApprovalCoverage(schema: string): Promise<PostureDimension> {
  const items: PostureCheckItem[] = [];
  let score = 0;
  const maxScore = 15;
...
async function assessModuleSecurity(schema: string, moduleCode: string): Promise<ModuleSecurityScore> {
  ...
  try {
    ...
    checks.hasRoles = roles.rows.length > 0; if (checks.hasRoles) score++;
    checks.hasPermissions = perms.rows.length > 0; if (checks.hasPermissions) score++;
    checks.hasActions = actions.rows.length > 0; if (checks.hasActions) score++;
    checks.hasSodRules = sod.rows.length > 0; if (checks.hasSodRules) score++;
    checks.hasApprovalMatrix = approval.rows.length > 0; if (checks.hasApprovalMatrix) score++;
    checks.hasOwnership = ownership.rows.length > 0; if (checks.hasOwnership) score++;
  } catch (err) {
...
export async function getPostureTrend(tenantId: string, days = 30): Promise<PostureTrendReport> {
  const schema = tenantSchema(tenantId);
  ...
  try {
    const { rows } = await safeQuery(
      `SELECT payload, created_at FROM "${schema}".agrc_event_log
       WHERE event_type = 'security_posture_result'
...
function generatePostureRecommendations(dimensions: PostureDimension[], moduleScores: ModuleSecurityScore[]): PostureRecommendation[] {
  const recs: PostureRecommendation[] = [];
  let priority = 1;
