// ============================================================================
// Shahin GRC — Agent Governance Service (F53-F55: Tool Permissions + HITL Gates)
//
...
function rowToPermission(r: any): ToolPermission {
  return {
...
function rowToGate(r: any): HITLGate {
  return {
    gateId: r.gate_id,
    agentId: r.agent_id,
    toolName: r.tool_name,
...
function rowToAuditEntry(r: any): AgentAuditEntry {
  return {
...
export async function resolveHITLGate(
  ...
  // Verify gate exists and is pending
  const gateRes = await safeQuery(
    `SELECT * FROM "${schema}".hitl_gates WHERE gate_id = $1`,
    [gateId]
  );

  if (gateRes.rows.length === 0) return null;

  const gate = gateRes.rows[0];
  if (gate.status !== 'pending') {
    throw new Error(`Gate ${gateId} is already ${gate.status} and cannot be resolved`);
  }

  // Check if gate has expired
  if (new Date(gate.expires_at) < new Date()) {
    await safeQuery(
      `UPDATE "${schema}".hitl_gates SET status = 'expired', updated_at = NOW() WHERE gate_id = $1`,
      [gateId]
    );
  ...
  await logAuditEntry(schema, {
    ...
    result: decision,
    details: { gateId, decidedBy, reason, toolName: gate.tool_name },
  });

  const eventType = decision === 'approved' ? 'gate.allowed' : 'gate.blocked';
  ...
  // Fetch and return updated gate
  const updatedRes = await safeQuery(
    `SELECT * FROM "${schema}".hitl_gates WHERE gate_id = $1`,
    [gateId]
  );

  return updatedRes.rows.length > 0 ? rowToGate(updatedRes.rows[0]) : null;
}
