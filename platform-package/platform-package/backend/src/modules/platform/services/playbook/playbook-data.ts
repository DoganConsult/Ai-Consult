/**
 * Playbook Data — Static configuration constants, interfaces, and permission helpers.
 *
 * Contains: NavItemDef, ROLE_PERMISSIONS, ALL_NAV_ITEMS, MODULE_I18N,
 * RESOURCE_I18N, ROLE_TO_SWIMLANES, hasPermission.
 *
 * This is a pure data module with no service dependencies beyond types.
 *
 * Requirements: 1.1, 2.1, 2.2
 */

import type { WorkflowEntry } from '../../../agrc-engine/services/engine/playbook.types';
...
// ─── Role → permissions mapping (mirrors frontend ROLE_PERMISSIONS) ───

const ALL_PLAYBOOK_PERMS = [
  'action.item.read', 'action.item.write', 'admin.config.read', 'admin.config.write',
  'admin.tenant.manage', 'admin.user.manage', 'ai.agent.read', 'ai.squad.read',
  'analytics.report.read', 'assessment.record.read', 'assessment.record.write',
  'audit.record.manage', 'audit.record.read', 'compliance.program.read', 'compliance.program.write',
  ...
  'incident.record.read', 'incident.record.write', 'integrations.connector.read',
  'messaging.channel.read', 'messaging.channel.write', 'policy.document.read', 'policy.document.write',
  'profile.record.read', 'quote.record.read', 'report.document.read', 'report.document.write',
  'risk.record.read', 'risk.record.write', 'task.item.read', 'task.item.write',
  'timeline.event.read', 'training.record.read', 'vendor.record.read', 'vendor.record.write',
  'workflow.autonomous.read', 'workflow.autonomous.write', 'workflow.instance.read', 'workflow.instance.write',
  'workspace.config.read',
];

export const ROLE_PERMISSIONS: Record<string, string[]> = {
  owner: ALL_PLAYBOOK_PERMS,
  admin: ALL_PLAYBOOK_PERMS,
  compliance_officer: [
    'policy.document.read', 'policy.document.write', 'risk.record.read', 'framework.record.read', 'framework.record.manage',
    'control.record.read', 'control.record.write', 'assessment.record.read', 'assessment.record.write',
    ...
    'timeline.event.read', 'task.item.read', 'task.item.write', 'messaging.channel.read', 'messaging.channel.write',
    'action.item.read', 'action.item.write', 'quote.record.read', 'ai.agent.read',
    'compliance.program.read', 'compliance.program.write', 'integrations.connector.read', 'training.record.read',
    'ai.squad.read', 'workflow.autonomous.read',
  ],
  risk_manager: [
    'policy.document.read', 'risk.record.read', 'risk.record.write', 'framework.record.read',
    'control.record.read', 'control.record.write', 'assessment.record.read',
    ...
    'timeline.event.read', 'task.item.read', 'task.item.write', 'messaging.channel.read', 'messaging.channel.write',
    'action.item.read', 'action.item.write', 'quote.record.read', 'ai.agent.read',
    'compliance.program.read', 'integrations.connector.read',
    'ai.squad.read', 'workflow.autonomous.read',
  ],
  auditor: [
    'policy.document.read', 'risk.record.read', 'framework.record.read', 'control.record.read',
    'assessment.record.read', 'audit.record.read', 'audit.record.manage',
    'analytics.report.read', 'copilot.assistant.read', 'report.document.read',
    'workspace.config.read', 'profile.record.read', 'workflow.instance.read',
    'timeline.event.read', 'task.item.read', 'messaging.channel.read', 'messaging.channel.write',
    'action.item.read', 'quote.record.read', 'ai.agent.read',
    'compliance.program.read', 'training.record.read',
    'ai.squad.read', 'workflow.autonomous.read',
  ],
  viewer: [
    'policy.document.read', 'risk.record.read', 'framework.record.read', 'control.record.read',
    ...
    'timeline.event.read', 'messaging.channel.read', 'quote.record.read',
    'compliance.program.read',
    'ai.squad.read',
  ],
  user: [
    'policy.document.read', 'risk.record.read', 'framework.record.read', 'control.record.read',
    'analytics.report.read', 'workspace.config.read', 'profile.record.read',
    'messaging.channel.read', 'quote.record.read', 'compliance.program.read',
  ],
  manager: [
    'policy.document.read', 'policy.document.write', 'risk.record.read', 'risk.record.write', 'framework.record.read',
    'control.record.read', 'control.record.write', 'assessment.record.read', 'assessment.record.write',
    'audit.record.read', 'analytics.report.read', 'copilot.assistant.read', 'report.document.read',
    'workspace.config.read', 'profile.record.read', 'workflow.instance.read', 'workflow.instance.write',
    'timeline.event.read', 'task.item.read', 'task.item.write', 'messaging.channel.read', 'messaging.channel.write',
    'action.item.read', 'action.item.write', 'quote.record.read', 'ai.agent.read',
    'compliance.program.read', 'integrations.connector.read',
    'ai.squad.read', 'workflow.autonomous.read', 'workflow.autonomous.write',
  ],
  approver: [
    'policy.document.read', 'risk.record.read', 'framework.record.read', 'control.record.read',
    'assessment.record.read', 'audit.record.read', 'analytics.report.read', 'copilot.assistant.read',
    'report.document.read', 'workspace.config.read', 'profile.record.read', 'workflow.instance.read',
    'timeline.event.read', 'task.item.read', 'messaging.channel.read', 'messaging.channel.write',
    'action.item.read', 'quote.record.read', 'ai.agent.read', 'compliance.program.read',
  ],
};
...
// ─── DB-driven nav items (Phase 9 — DB-driven UI) ────────────────────────────

import { getNavigationItems, getNavigationForPermissions } from '../../../../platform/dos/shell/navigation/navigation-registry.service';

/** DB-driven nav items. Falls back to hardcoded ALL_NAV_ITEMS if DB unavailable. */
export async function getNavItemsForTenant(tenantId: string): Promise<NavItemDef[]> {
  const dbItems = await getNavigationItems(tenantId);
  if (dbItems.length > 0) {
    return dbItems.map(item => ({
      ...
      lifecyclePhase: item.lifecyclePhase ?? 'plan',
    }));
  }
  return ALL_NAV_ITEMS;
}

/** DB-driven nav items filtered by user permissions. */
export async function getNavItemsForUser(tenantId: string, userPermissions: string[]): Promise<NavItemDef[]> {
  const dbItems = await getNavigationForPermissions(tenantId, userPermissions);
  if (dbItems.length > 0) {
    return dbItems.map(item => ({
      icon: item.icon,
      labelKey: item.labelKey,
  ...
  return ALL_NAV_ITEMS.filter(item => permSet.has(item.requiredPermission));
}


// ─── Bilingual name/description lookup for nav items ───

/** Maps labelKey to bilingual name and description for ModuleEntry enrichment. */
...
// ─── Role → workflow swimlane mapping ───
