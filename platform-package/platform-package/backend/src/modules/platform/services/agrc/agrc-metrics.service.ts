// ============================================
// Shahin GRC — AGRC-OS Metrics Service
...
export async function computeMetrics(tenantId: string, hours: number = 24): Promise<AGRCMetrics> {
  ...
  try {
    ...
    const total = ccmRes.rows[0]?.total || 1;
    staleControlPct = Math.round((ccmRes.rows[0]?.stale || 0) / total * 10000) / 100;
  } catch { /* table may not exist */ }
  ...
  try {
    ...
    eventCount = eventRes.rows[0]?.total || 0;
    criticalEvents = eventRes.rows[0]?.critical || 0;
  } catch { /* table may not exist */ }
...
export async function getMetricsHistory(tenantId: string, limit: number = 30): Promise<any[]> {
  const schema = tenantSchema(tenantId);
  try {
