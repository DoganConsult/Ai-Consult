...
// ================================================================
// AGRC-OS Integration Service — Bridges All Autonomy Gaps
//
...
import { eventBus, PlatformEvent } from '../event/event-bus.service';
...
interface Edge {
  ...
  raci_mirror: Record<string, any>;
  capabilities: string[];
  auto_actions: string[];
  enabled: boolean;
}

// Trigger types that activation rules support
type TriggerType =
  | 'overdue_task'
  | 'evidence_gap'
  | 'risk_threshold'
...
const TRIGGER_EVALUATORS: Record<TriggerType, (schema: string, config: Record<string, any>) => Promise<{ triggered: boolean; context: Record<string, any> }>> = {
  overdue_task: async (schema, config) => {
    const threshold = config.thresholdCount || 5;
    const res = await safeQuery(
      `SELECT COUNT(*)::int AS n FROM "${schema}".tasks WHERE status = 'open' AND due_date < NOW()`,
    );
  ...
  risk_threshold: async (schema, config) => {
    ...
    const res = await safeQuery(
      `SELECT COUNT(*)::int AS n, MAX(risk_score)::int AS max_score
       FROM "${schema}".risks WHERE risk_score > $1 AND status != 'closed'`,
      [maxScore],
    );
    const count = res.rows[0]?.n || 0;
    return { triggered: count > 0, context: { highRiskCount: count, maxScore: res.rows[0]?.max_score || 0 } };
  },

  control_failure: async (schema, config) => {
    const threshold = config.thresholdCount || 1;
    const res = await safeQuery(
      `SELECT COUNT(*)::int AS n FROM "${schema}".ucf_controls WHERE test_status = 'failed'`,
    );
    const count = res.rows[0]?.n || 0;
  ...
  compliance_drop: async (schema, config) => {
    ...
    const res = await safeQuery(
      `SELECT AVG(score)::numeric(5,1) AS avg FROM "${schema}".compliance_assessments
       WHERE created_at > NOW() - INTERVAL '30 days'`,
    );
    const avg = Number(res.rows[0]?.avg || 100);
  ...
  workload_spike: async (schema, config) => {
    ...
    const res = await safeQuery(
      `SELECT assigned_to, COUNT(*)::int AS n FROM "${schema}".tasks
...
export async function evaluateActivationRules(tenantId: string): Promise<{ evaluated: number; triggered: number; actions: string[] }> {
  ...
  const shadows = await safeQuery(
    `SELECT s.shadow_id, s.user_id, s.team_id, s.agent_name, s.activation_mode, s.enabled,
  ...
  for (const shadow of shadows.rows || []) {
    const rules = await safeQuery(
      `SELECT * FROM "${schema}".agent_activation_rules
    ...
    for (const rule of rules.rows || []) {
      ...
      try {
        ...
        await safeQuery(
          `UPDATE "${schema}".agent_activation_rules
        ...
        await safeQuery(
          `UPDATE "${schema}".member_agent_shadows
...
async function executeActivationAction(
  tenantId: string, _schema: string,
  shadow: any, rule: ActivationRule, triggerContext: Record<string, any>,
): Promise<string> {
  ...
  switch (actionType) {
    case 'run_agent': {
      ...
      if (agentId) {
        const { runAgent } = await import('../../../ai/services/agents/core/agent-runner.service');
        ...
        await eventBus.publish({
          eventType: 'agent.activation_triggered' as any,
...
function mapShadowToAgent(shadow: any): string | null {
  ...
  for (const cap of capabilities) {
    if (capMap[cap]) return capMap[cap];
  }

  // Fallback: match by name
  if (name.includes('compliance')) return 'A01';
  if (name.includes('risk')) return 'A07';
  if (name.includes('evidence')) return 'A05';
  if (name.includes('policy')) return 'A08';
  if (name.includes('audit')) return 'A10';
  if (name.includes('vendor')) return 'A09';
  if (name.includes('incident')) return 'A07';
...
export function initEventDrivenAgentTriggers(): void {
  if (_eventSubscriptionsInitialized) return;
  ...
  for (const [eventType, agentIds] of Object.entries(EVENT_AGENT_MAP)) {
    eventBus.subscribe(eventType, `agrc-os-reactive-${eventType}`, async (event: PlatformEvent) => {
      const tenantId = event.tenantId;
      if (!tenantId) return;

      if (agentIds.includes('feedback')) {
        // Gap 6: feedback loop
        await processTaskFeedback(tenantId, event).catch((err: unknown) =>
          logger.warn(`[AGRC-OS] Feedback processing failed: ${toErrorMessage(err)}`)
        );
        return;
      }
      ...
      for (const agentId of agentIds) {
        ...
        if (reactiveCooldown.has(cacheKey)) continue;
...
const reactiveCooldown = new Map<string, number>();
...
export async function persistHandoff(
  ...
  handoffType: string, priority: string,
  ...
  await safeQuery(
    `INSERT INTO "${schema}".agent_handoffs (id, from_agent, to_agent, handoff_type, priority, status, payload, created_at)
     VALUES ($1, $2, $3, $4, $5, 'pending', $6, NOW())
     ON CONFLICT (id) DO NOTHING`,
    [id, fromAgent, toAgent, handoffType, priority, JSON.stringify(payload)],
  );

  return id;
}

export async function getPersistentPendingHandoffs(tenantId: string, agentId: string): Promise<any[]> {
  const schema = tenantSchema(tenantId);
  const res = await safeQuery(
    `SELECT * FROM "${schema}".agent_handoffs
...
export async function completePersistentHandoff(tenantId: string, handoffId: string, result: Record<string, any>): Promise<void> {
  const schema = tenantSchema(tenantId);
  await safeQuery(
    `UPDATE "${schema}".agent_handoffs SET status = 'completed', completed_at = NOW(),
       payload = payload || $2::jsonb WHERE id = $1`,
    [handoffId, JSON.stringify({ result })],
  );
}

export async function getActiveCycleContext(tenantId: string): Promise<any | null> {
  const schema = tenantSchema(tenantId);
  const res = await safeQuery(
    `SELECT * FROM "${schema}".agent_cycle_summaries
...
export async function evaluateWorkloadDelegation(tenantId: string): Promise<{ evaluated: number; delegated: number; details: string[] }> {
  const schema = tenantSchema(tenantId);
  ...
  const shadows = await safeQuery(
    `SELECT s.shadow_id, s.user_id, s.team_id, s.agent_name, s.activation_mode,
  ...
  for (const shadow of shadows.rows || []) {
    ...
    const taskRes = await safeQuery(
      `SELECT COUNT(*)::int AS open_tasks FROM "${schema}".tasks
       WHERE assigned_to = $1 AND status = 'open'`,
      [shadow.user_id],
    );
    const openTasks = taskRes.rows[0]?.open_tasks || 0;

    // Threshold: configurable per shadow or default 15
    const threshold = 15;
    if (openTasks < threshold) continue;

    // Check if delegation already active
    const existingGrant = await safeQuery(
      `SELECT grant_id FROM "${schema}".delegation_grants
       WHERE user_id = $1 AND revoked_at IS NULL AND expires_at > NOW()
       LIMIT 1`,
      [shadow.user_id],
    );
    if (existingGrant.rows.length > 0) continue;
...
async function autoDelegateToAgent(
  tenantId: string, userId: string, agentId: string,
  teamId: string, context: Record<string, any>,
): Promise<void> {
  try {
    const { createDelegationGrant } = await import('../../../ai/services/delegation/agent-delegation.service');
    await createDelegationGrant(
      tenantId,
      userId,
      `AGENT-${agentId}`,
      ['assessment', 'evidence_upload', 'risk_seeding'],
      24 * 60, // 24 hours in minutes
    );

    await eventBus.publish({
      eventType: 'agent.auto_delegated' as any,
      tenantId, sourceService: 'agrc-os-integration',
      severity: 'info',
...
export async function seedAgentTargetsFromProfile(tenantId: string): Promise<{ seeded: number; targets: string[] }> {
  const targets: string[] = [];

  // Load tenant profile
  const tenantRes = await safeQuery(
    `SELECT org_type, primary_sector_id, sector_ids, critical_infrastructure,
  ...
  if (!tenantRes.rows.length) return { seeded: 0, targets: [] };
  const profile = tenantRes.rows[0];
  ...
  const upsertTarget = async (agentId: string, targetType: string, config: Record<string, any>, priority: string) => {
    await safeQuery(
      `INSERT INTO "${schema}".agent_monitoring_targets (agent_id, target_type, target_config, priority)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (agent_id, target_type) DO UPDATE SET target_config = $3, priority = $4`,
      [agentId, targetType, JSON.stringify(config), priority],
    );
...
async function processTaskFeedback(tenantId: string, event: PlatformEvent): Promise<void> {
  const schema = tenantSchema(tenantId);
  ...
  const taskRes = await safeQuery(
    `SELECT title, description, assigned_to, status FROM "${schema}".tasks WHERE task_id = $1`,
    [entityId],
  );
  if (!taskRes.rows.length) return;
  ...
  await safeQuery(
    `INSERT INTO "${schema}".agent_feedback_log (agent_id, task_id, feedback_type, task_title, responded_by)
     VALUES ($1, $2, $3, $4, $5)`,
    [agentId, entityId, isCompleted ? 'accepted' : isRejected ? 'rejected' : 'other',
  ...
  if (isCompleted) {
    await safeQuery(
      `INSERT INTO "${schema}".agent_priority_weights (agent_id, total_accepted, acceptance_rate)
       VALUES ($1, 1, 100)
    ...
    await safeQuery(
      `INSERT INTO "${schema}".agent_priority_weights (agent_id, total_rejected, acceptance_rate)
       VALUES ($1, 1, 0)
       ON CONFLICT (agent_id) DO UPDATE SET
...
export async function runAgrcOsIntegrationCycle(tenantId: string): Promise<{
  activationRules: { evaluated: number; triggered: number };
  workloadDelegation: { evaluated: number; delegated: number };
}> {
  // Gap 1: Evaluate activation rules
  const activation = await swallowDefault(EC.FALLBACK_QUERY, { evaluated: 0, triggered: 0, actions: [] as never[] }, evaluateActivationRules(tenantId), { tenantId: tenantId, operation: 'fallback query' });

  // Gap 4: Evaluate workload delegation
  const workload = await swallowDefault(EC.FALLBACK_QUERY, { evaluated: 0, delegated: 0, details: [] as never[] }, evaluateWorkloadDelegation(tenantId), { tenantId: tenantId, operation: 'fallback query' });
