...
// ============================================
// Shahin GRC — GRC Integrity Guard
...
export async function runIntegrityGuard(tenantId: string): Promise<IntegrityResult> {
  ...
  try {
    const orphanControls = await safeQuery(
    ...
    if (orphanControls.rows.length > 0) {
      ...
      const defaultPolicy = await safeQuery(
        `SELECT policy_id, title FROM "${schema}".policies
         WHERE title ILIKE '%information security%' OR title ILIKE '%security policy%'
         ORDER BY created_at ASC LIMIT 1`
      );
      const fallbackPolicy = defaultPolicy.rows[0] || (await safeQuery(
        `SELECT policy_id, title FROM "${schema}".policies ORDER BY created_at ASC LIMIT 1`
      )).rows[0];

      if (fallbackPolicy) {
        for (const ctrl of orphanControls.rows) {
          await safeQuery(
            `UPDATE "${schema}".controls SET policy_id = $1 WHERE control_id = $2`,
            [fallbackPolicy.policy_id, ctrl.control_id]
          );
  ...
  try {
    const orphanPolicies = await safeQuery(
    ...
    if (orphanPolicies.rows.length > 0) {
      ...
      const frameworks = await safeQuery(
        `SELECT framework_id FROM "${schema}".frameworks LIMIT 20`
      );
      const fwIds = frameworks.rows.map((f: GenericRow) => f.framework_id);

      if (fwIds.length > 0) {
        for (const pol of orphanPolicies.rows) {
          await safeQuery(
            `UPDATE "${schema}".policies SET frameworks = $1 WHERE policy_id = $2`,
            [fwIds, pol.policy_id]
          );
  ...
  try {
    const uncategorized = await safeQuery(
    ...
    for (const risk of uncategorized.rows) {
      await safeQuery(
        `UPDATE "${schema}".risks SET category = 'operational' WHERE risk_id = $1`,
        [risk.risk_id]
      );
  ...
  try {
    const orphanEvidence = await safeQuery(
      `SELECT e.evidence_id, e.title
    ...
    if (orphanEvidence.rows.length > 0) {
      ...
      const generalControl = await safeQuery(
        `SELECT control_id FROM "${schema}".controls ORDER BY created_at ASC LIMIT 1`
      );

      if (generalControl.rows[0]) {
        for (const ev of orphanEvidence.rows) {
          await safeQuery(
            `UPDATE "${schema}".evidence SET control_id = $1 WHERE evidence_id = $2`,
            [generalControl.rows[0].control_id, ev.evidence_id]
          );
          evidenceFixed++;
        }
  ...
  try {
    const staleIncidents = await safeQuery(
  ...
  try {
    const unlinkedFindings = await safeQuery(
      `SELECT ai.item_id, ai.control_node_id, ai.status
  ...
  try {
    const unownedControls = await safeQuery(
    ...
    const unownedRisks = await safeQuery(
    ...
    const unownedEvidence = await safeQuery(
    ...
    if (raciFixed > 0) {
      await swallow(EC.EVENT_BUS, eventBus.publish({
        eventType: 'raci.auto_assigned' as any, tenantId,
        sourceService: 'grc-integrity-guard',
        severity: 'info',
  ...
  try {
    const stale = await safeQuery(
      `SELECT bcp_id FROM "${schema}".bcp_plans
    ...
    for (const p of stale.rows) {
      await safeQuery(
        `UPDATE "${schema}".bcp_plans SET status = 'review_required', updated_at = NOW() WHERE bcp_id = $1 AND status = 'approved'`,
        [p.bcp_id]
      );
      incidentsFixed++;
    }
  ...
  try {
    const expired = await safeQuery(
      `SELECT dd_id, vendor_id FROM "${schema}".vendor_due_diligence
    ...
    for (const dd of expired.rows) {
      await safeQuery(
        `UPDATE "${schema}".vendor_due_diligence SET status = 'expired', updated_at = NOW() WHERE dd_id = $1`,
        [dd.dd_id]
      );
      await swallow(EC.EVENT_BUS, eventBus.publish({
        eventType: 'vendor.dd_expired' as any, tenantId,
        sourceService: 'grc-integrity-guard',
  ...
  try {
    const overdue = await safeQuery(
      `SELECT assignment_id, user_id FROM "${schema}".training_assignments
    ...
    for (const a of overdue.rows) {
      await safeQuery(
        `UPDATE "${schema}".training_assignments SET status = 'escalated', updated_at = NOW() WHERE assignment_id = $1 AND status IN ('assigned','in_progress')`,
        [a.assignment_id]
      );
      incidentsFixed++;
    }
  ...
  try {
    const aging = await safeQuery(
      `SELECT near_miss_id FROM "${schema}".near_miss_reports
    ...
    for (const nm of aging.rows) {
      await safeQuery(
        `UPDATE "${schema}".near_miss_reports SET status = 'under_review', updated_at = NOW() WHERE near_miss_id = $1 AND status = 'reported'`,
        [nm.near_miss_id]
      );
      incidentsFixed++;
    }
  ...
  if (totalFixed > 0 || unfixable.length > 0) {
    ...
    logger.info(
      `[GRC Integrity] tenant ${tenantId}: ${totalFixed} fixes (${controlsFixed} controls, ` +
