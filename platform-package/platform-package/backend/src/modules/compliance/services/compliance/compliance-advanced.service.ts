// ============================================
// Shahin GRC — Compliance Advanced Service
...
export async function updateRegulatoryChangeStatus(
  ...
  const completedAt = status === "closed" || status === "verified" ? "NOW()" : "completed_at";
  const result = await safeQuery(
  ...
  if (result.rows.length === 0) throw new Error("Regulatory change not found");
  return result.rows[0];
}
  ...
  const result = await safeQuery(
    `UPDATE "${schema}".regulatory_changes
     SET impact_assessment = $1,
         ...
         affected_frameworks = COALESCE($3, affected_frameworks),
         affected_controls = COALESCE($4, affected_controls),
         affected_policies = COALESCE($5, affected_policies),
         status = CASE WHEN status = 'identified' THEN 'impact_assessed' ELSE status END,
         updated_at = NOW()
     WHERE change_id = $6
     RETURNING *`,
    [
      JSON.stringify(impactData.assessment || impactData),
      impactData.impactLevel || null,
      impactData.affectedFrameworks ? JSON.stringify(impactData.affectedFrameworks) : null,
      impactData.affectedControls ? JSON.stringify(impactData.affectedControls) : null,
      impactData.affectedPolicies ? JSON.stringify(impactData.affectedPolicies) : null,
      changeId,
    ]
  );
  if (result.rows.length === 0) throw new Error("Regulatory change not found");
...
export async function updateObligation(
  ...
  const fieldMap: Record<string, any> = {
    title: data.title,
    obligation_text: data.obligationText,
    source_regulation: data.sourceRegulation,
    jurisdiction: data.jurisdiction,
    framework_id: data.frameworkId,
    control_ids: data.controlIds ? JSON.stringify(data.controlIds) : undefined,
    owner: data.owner,
    ...
    evidence_ids: data.evidenceIds ? JSON.stringify(data.evidenceIds) : undefined,
  };
  ...
  const result = await safeQuery(
    `UPDATE "${schema}".compliance_obligations SET ${setClauses.join(", ")} WHERE obligation_id = $${paramIdx} RETURNING *`,
    params
  );
  if (result.rows.length === 0) throw new Error("Obligation not found");
  return result.rows[0];
}
...
export async function executeTestPlan(
  ...
  const controlIds: string[] = plan.control_ids || [];

  // Test each linked control by checking evidence freshness
  let passCount = 0;
  let failCount = 0;
  const controlResults: Record<string, any>[] = [];

  for (const controlId of controlIds) {
    const evRes = await safeQuery(
      `SELECT COUNT(*)::int AS cnt FROM "${schema}".evidence WHERE control_id = $1`,
      [controlId]
    );
    const hasEvidence = (evRes.rows[0]?.cnt || 0) > 0;
    if (hasEvidence) passCount++;
    else failCount++;
    controlResults.push({ controlId, hasEvidence, result: hasEvidence ? "pass" : "fail" });
  }

  const overallResult = failCount === 0 ? "pass" : passCount === 0 ? "fail" : "partial";
  ...
  const daysToAdd = frequencyDays[plan.frequency] || 90;
  ...
  const updated = await safeQuery(
    `UPDATE "${schema}".control_test_plans
     ...
     WHERE plan_id = $3
     RETURNING *`,
    [overallResult, nextRun.toISOString().split("T")[0], planId]
  );
...
export async function updateFilingStatus(
  ...
  const schema = tenantSchema(tenantId);
  const validStatuses = ["pending", "in_progress", "submitted", "accepted", "rejected", "overdue"];
  if (!validStatuses.includes(status)) {
    throw new Error(`Invalid status. Must be one of: ${validStatuses.join(", ")}`);
  }

  // Auto-set submitted_date when status transitions to submitted
  const submittedClause = status === "submitted" ? ", submitted_date = CURRENT_DATE" : "";

  const result = await safeQuery(
    `UPDATE "${schema}".regulatory_filings
     SET status = $1, updated_at = NOW()${submittedClause}
     WHERE filing_id = $2
     RETURNING *`,
    [status, filingId]
  );
  if (result.rows.length === 0) throw new Error("Filing not found");
  return result.rows[0];
}
