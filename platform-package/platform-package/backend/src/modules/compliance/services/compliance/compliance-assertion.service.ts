// @ts-nocheck
// ============================================================================
// Shahin GRC — Compliance Assertion Engine (F22-23)
//
...
export async function evaluateControlAssertion(
  ...
  // ── 1. Evidence coverage: avg of composite_scores from evidence_scores ──
  const evidenceRes = await safeQuery(
    `SELECT e.id, e.title, e.status, e.created_at, e.expiry_date
  ...
  const evidenceIds = evidenceRows.map((e: GenericRow) => e.id);
  ...
  if (evidenceRows.length > 0) {
    const scoreRes = await safeQuery(
      `SELECT evidence_id, composite_score
       FROM "${schema}".evidence_scores
       WHERE evidence_id = ANY($1)`,
      [evidenceIds]
    );
    for (const row of scoreRes.rows) {
      evidenceScores[row.evidence_id] = row.composite_score;
    }
  ...
  // ── 10. Emit event ──
  eventBus.publish("assertion.evaluated", {
    tenantId,
    controlId,
    assertionId,
    status,
...
export async function evaluateFrameworkAssertions(
  ...
  eventBus.publish("assertion.framework_evaluated", {
    tenantId,
    frameworkId,
 ...
 */
function rowToAssertion(r: any): ComplianceAssertion {
  return {
