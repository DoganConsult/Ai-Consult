...
import { testControl } from './compliance.service';
import { getFirstRow } from '../../../../utils/db-utils';
import type { ComplianceTestDefinition, TestStep, TestExecutionResult } from './compliance-as-code.types';
import { evalCondition, compareResult } from './compliance-expression-parser';
...
export async function executeTest(
  ...
  let overallStatus: 'pass' | 'fail' | 'partial' | 'error' = 'pass';
  ...
  try {
    ...
    for (const step of definition.steps) {
      ...
      try {
        stepResult = await executeStep(tenantId, definition.controlId, step);

        // Compare result with expected
        const matches = compareResult(stepResult, step.expected);
        stepStatus = matches ? 'pass' : 'fail';

        if (!matches && (step.failFast || options.failOnFirstError)) {
          overallStatus = 'fail';
          break;
        }

        if (!matches) {
          overallStatus = overallStatus === 'pass' ? 'fail' : overallStatus;
        }
      } catch (err: unknown) {
        stepStatus = 'error';
        ...
        if (step.failFast || options.failOnFirstError) {
    ...
    const result: TestExecutionResult = {
      ...
      steps: stepResults,
      overallResult: overallStatus === 'pass' ? 'pass' : 'fail',
      error: executionError,
      durationMs: Date.now() - startTime,
    };

    // If not dry run, record result and publish events
    if (!options.dryRun) {
      await recordTestResult(tenantId, result);

      // Publish control.failed event if test failed
      if (result.overallResult === 'fail') {
        await eventBus.publish({
          eventType: 'control.failed',
          tenantId,
          sourceService: 'compliance-as-code',
          entityType: 'control',
          entityId: definition.controlId,
          severity: 'warning',
          payload: {
            testId,
            testName: definition.name,
            failureReason: executionError || 'Test assertions failed',
            failedSteps: stepResults.filter((s) => s.status === 'fail' || s.status === 'error'),
          },
        });
      }
    }
  ...
  switch (step.type) {
    ...
    case 'run_script': {
      ...
      if (scriptType === 'javascript') {
        ...
        const variables: Record<string, any> = { controlId, tenantId };
        try {
    ...
    default:
      throw new Error(`Unknown step type: ${(step as Record<string, unknown>).type}`);
  }
}
