...
export function tokenize(input: string): Token[] {
  ...
  while (i < input.length) {
    ...
    // ${variable} interpolation syntax — treat as identifier
    if (input[i] === '$' && i + 1 < input.length && input[i + 1] === '{') {
      i += 2; // skip ${
      let ident = '';
      while (i < input.length && input[i] !== '}') { ident += input[i]; i++; }
    ...
    if (i + 1 < input.length) {
      ...
      if (two === '&&') { tokens.push({ type: 'AND', value: '&&', pos }); i += 2; continue; }
      if (two === '||') { tokens.push({ type: 'OR', value: '||', pos }); i += 2; continue; }
    }

    // Single-character operators
    if (input[i] === '>') { tokens.push({ type: 'GT', value: '>', pos }); i++; continue; }
    if (input[i] === '<') { tokens.push({ type: 'LT', value: '<', pos }); i++; continue; }
    ...
    if (input[i] === '+') { tokens.push({ type: 'PLUS', value: '+', pos }); i++; continue; }
    if (input[i] === '-') { tokens.push({ type: 'MINUS', value: '-', pos }); i++; continue; }
    if (input[i] === '*') { tokens.push({ type: 'STAR', value: '*', pos }); i++; continue; }
    if (input[i] === '/') { tokens.push({ type: 'SLASH', value: '/', pos }); i++; continue; }
    if (input[i] === '!') { tokens.push({ type: 'NOT', value: '!', pos }); i++; continue; }

    // Identifier or keyword (AND, OR, NOT, true, false, contains, etc.)
    if (/[a-zA-Z_]/.test(input[i])) {
      let ident = '';
      while (i < input.length && /[a-zA-Z0-9_]/.test(input[i])) { ident += input[i]; i++; }
      const upper = ident.toUpperCase();
      if (upper === 'AND') { tokens.push({ type: 'AND', value: 'AND', pos }); }
      else if (upper === 'OR') { tokens.push({ type: 'OR', value: 'OR', pos }); }
      else if (upper === 'NOT') { tokens.push({ type: 'NOT', value: 'NOT', pos }); }
      else if (ident === 'true') { tokens.push({ type: 'BOOLEAN', value: true, pos }); }
      else if (ident === 'false') { tokens.push({ type: 'BOOLEAN', value: false, pos }); }
  ...
  private callOrAccess(): unknown {
    ...
    while (this.peek().type === 'DOT') {
      ...
      if (this.peek().type === 'LPAREN') {
        ...
        // Property access
        value = value != null ? (value as Record<string, any>)[propName] : undefined;
      }
    }
 ...
 */
export function evalCondition(condition: string, variables: Record<string, any>): boolean {
 ...
 */
export function compareResult(actual: any, expected: any): boolean {
  if (expected === undefined) return true; // No expectation = always pass

  // Simple equality check
  if (typeof expected === 'boolean') {
    return (actual as Record<string, any>)?.passed === expected;
  }
