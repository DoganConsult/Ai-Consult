...
export async function summarize(tenantId: string, entityId: string): Promise<{ summary: string; keyPoints: string[]; confidence: number }> {
  ...
  const result = await safeQuery(
    `SELECT ca.title, ca.framework, ca.status, ca.score, ca.assessment_date, ca.assessor
     FROM ${schema}.compliance_assessments ca WHERE ca.id = $1`,
    [entityId]
  );
...
export async function classify(tenantId: string, entityId: string): Promise<{ category: string; confidence: number; alternatives: Array<{ category: string; confidence: number }> }> {
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT framework, score, status FROM ${schema}.compliance_assessments WHERE id = $1`,
    [entityId]
  );
  const r = result.rows[0];
...
export async function scoreRisk(tenantId: string, entityId: string): Promise<{ score: number; factors: Array<{ factor: string; weight: number; value: number }>; recommendation: string }> {
  ...
  const result = await safeQuery(
    `SELECT ca.score, ca.framework,
     ...
     FROM ${schema}.compliance_assessments ca
     LEFT JOIN ${schema}.compliance_items ci ON ci.assessment_id = ca.id
     WHERE ca.id = $1
...
export async function analyzeGaps(tenantId: string): Promise<{ gaps: Array<{ area: string; currentState: string; targetState: string; severity: string; recommendation: string }> }> {
  ...
  const [lowScore, _nonCompliant, overdue] = await Promise.all([
    safeQuery(`SELECT framework, score FROM ${schema}.compliance_assessments WHERE score < 70 AND status != 'closed' ORDER BY score ASC LIMIT 5`, []),
    safeQuery(`SELECT framework, COUNT(*) AS cnt FROM ${schema}.compliance_assessments ca JOIN ${schema}.compliance_items ci ON ci.assessment_id = ca.id WHERE ci.status = 'non_compliant' GROUP BY framework ORDER BY cnt DESC LIMIT 5`, []),
    safeQuery(`SELECT COUNT(*) AS cnt FROM ${schema}.compliance_assessments WHERE next_review_date < NOW() AND status != 'closed'`, []),
  ]);
  const gaps = [];
...
export async function generateReport(tenantId: string, params: { period?: string; format?: string }): Promise<{ title: string; sections: Array<{ heading: string; content: string; data?: Record<string, unknown> }> }> {
  ...
  const [summary, byFramework] = await Promise.all([
    safeQuery(`SELECT COUNT(*) AS total, ROUND(AVG(score),1) AS avg_score, COUNT(*) FILTER (WHERE score>=70) AS passing FROM ${schema}.compliance_assessments`, []),
    safeQuery(`SELECT framework, COUNT(*) AS cnt, ROUND(AVG(score),1) AS avg_score FROM ${schema}.compliance_assessments GROUP BY framework ORDER BY avg_score ASC LIMIT 10`, []),
  ]);
  ...
  return {
    ...
    sections: [
      ...
      { heading: 'Framework Coverage', content: byFramework.rows.map(r => `${r.framework}: ${r.avg_score}% avg (${r.cnt} assessments)`).join('; ') || 'No data.', data: { frameworks: byFramework.rows } },
    ],
  };
}
