...
import { pct, computeMaturity, ctx } from "../misc/compliance.utils";
...
export async function getFrameworksRegister(tenantId: string, options?: FrameworksRegisterOptions): Promise<any> {
  ...
  return fwRes.rows.map((f: GenericRow) => {
    const agg = aggByFw.get(f.framework_id) || { controlsMapped: 0, implemented: 0, withEvidence: 0, tested: 0 } as string;
...
export async function getFrameworkDetail(tenantId: string, frameworkCode: string): Promise<any> {
  const { schema } = ctx(tenantId);
  // Framework row
...
export async function getFrameworkComparison(tenantId: string): Promise<any> {
...
export async function getDomainsRegister(tenantId: string, frameworkId?: string): Promise<any> {
  const { schema } = ctx(tenantId);
  ...
  const now = new Date();
  const __overdueTaskIds = new Set(remRes.rows.filter((t: GenericRow) => t.due_date && new Date(t.due_date) < now).map((t: GenericRow) => t.task_id));
  ...
  for (const d of domRes.rows) {
    ...
    const oblIds = oblRes.rows.map((o: unknown) => o.node_id);
 ...
 */
export async function getDomainSummary(tenantId: string, domainId: string): Promise<any> {
  const { schema } = ctx(tenantId);
...
export async function getDomainDetail(tenantId: string, nodeId: string): Promise<any> {
  const { schema } = ctx(tenantId);
  ...
  return {
    ...
    obligations: oblRes.rows.map((o: unknown) => {
...
export async function getObligationsRegister(tenantId: string, frameworkId?: string): Promise<any> {
  const { schema } = ctx(tenantId);
  ...
  return oblRes.rows.map((o: unknown) => {
...
export async function getObligationDetail(tenantId: string, nodeId: string): Promise<any> {
  const { schema } = ctx(tenantId);
...
export async function mapControlToObligation(tenantId: string, nodeId: string, controlId: string): Promise<any> {
  const { schema } = ctx(tenantId);
  const res = await safeQuery(
...
export async function mapEvidenceToObligation(tenantId: string, nodeId: string, evidenceId: string): Promise<any> {
  const { schema } = ctx(tenantId);
  ...
  eventBus.publish({ eventType: 'compliance.evidence_linked', tenantId, sourceService: 'ComplianceWorkspaceService', severity: 'info', payload: { nodeId, evidenceId, controlId: ctrl.control_id } });
...
export async function updateObligation(tenantId: string, nodeId: string, data: { owner?: string; status?: string }): Promise<any> {
  const sets: string[] = [];
  const params: unknown[] = [];
  let idx = 1;
  if (data.owner) { sets.push(`owner = $${idx}`); params.push(data.owner); idx++; }
  ...
  if (row) {
    if (data.owner) eventBus.publish({ eventType: 'compliance.owner_assigned', tenantId, sourceService: 'ComplianceWorkspaceService', severity: 'info', payload: { nodeId, owner: data.owner } });
    if (data.status) eventBus.publish({ eventType: 'compliance.obligation_updated', tenantId, sourceService: 'ComplianceWorkspaceService', severity: 'info', payload: { nodeId, status: data.status } });
  }
  return row;
}
...
export async function importObligations(tenantId: string, obligations: Array<{
  ...
  if (imported > 0) {
    eventBus.publish({ eventType: 'compliance.obligation_created', tenantId, sourceService: 'ComplianceWorkspaceService', severity: 'info', payload: { importedCount: imported, source: 'bulk_import' } });
  }
