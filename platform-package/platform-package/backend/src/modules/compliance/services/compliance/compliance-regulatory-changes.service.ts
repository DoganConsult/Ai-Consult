...
export async function getRegulatoryChanges(tenantId: string, status?: string): Promise<any> {
...
export async function updateRegulatoryChangeStatus(tenantId: string, changeId: string, status: string): Promise<any> {
  const { schema } = ctx(tenantId);
 ...
 */
export async function getRegulatoryChangeImpact(tenantId: string, changeId: string): Promise<any> {
  const { schema } = ctx(tenantId);
