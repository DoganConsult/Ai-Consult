...
import { resolveImpact } from '../misc-canonical/regulation-compiler.service';
...
export async function detectDelta(instrumentId: string): Promise<RegulatoryDelta | null> {
  ...
  const addedNodes = Array.from(currentNodeIds).filter(id => !previousNodeIds.has(id)) as string;
...
export async function computeTenantImpacts(delta: RegulatoryDelta): Promise<TenantImpact[]> {
  ...
  for (const tenant of tenants.rows) {
    ...
    try {
      ...
      const tenantControlNodes = controlsResult.rows.flatMap((c: unknown) => c.mapped_registry_nodes || []);
