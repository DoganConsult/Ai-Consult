// @ts-nocheck
// ============================================
// Shahin GRC — Workflow Template Service
...
import { MODULE_WORKFLOW_MAP } from '../../../../config/module-workflow-map';
import { getWorkflowTemplateByCode } from '../../../../data/workflow-templates/index';
import { isCanonicalModuleCode, CanonicalModuleCode } from '../../../../config/canonical-modules';
...
export function deserializeTemplateDefinition(json: string): WorkflowDefinition {
  ...
  return {
    nodes: parsed.nodes,
    edges: parsed.edges,
    swimlanes: Array.isArray(parsed.swimlanes) ? parsed.swimlanes : [],
...
export async function instantiateTemplate(
  ...
  const inMemory = PREDEFINED_TEMPLATES.find(t => t.templateKey === templateKey);
  if (inMemory) {
    templateName = inMemory.name_en;
    defJson = JSON.stringify(inMemory.definition);
  } else {
    const dbResult = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(
      `SELECT name, definition FROM "${schema}".workflow_templates WHERE template_id::text = $1 OR name = $1`,
      [templateKey]
    ), { tenantId: tenantId, operation: 'query workflow_templates' });
    if (dbResult.rows.length > 0) {
      templateName = dbResult.rows[0].name;
      defJson = typeof dbResult.rows[0].definition === 'string'
        ? dbResult.rows[0].definition
        : JSON.stringify(dbResult.rows[0].definition);
    } else {
      const wfResult = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(
        `SELECT name, definition FROM "${schema}".workflows WHERE workflow_id::text = $1 AND status = 'template'`,
        [templateKey]
      ), { tenantId: tenantId, operation: 'query workflows' });
      if (wfResult.rows.length > 0) {
        templateName = wfResult.rows[0].name;
        defJson = typeof wfResult.rows[0].definition === 'string'
          ? wfResult.rows[0].definition
          : JSON.stringify(wfResult.rows[0].definition);
      } else {
  ...
  for (const [key, value] of Object.entries(params)) {
    defJson = defJson.replace(new RegExp(`\\$\\{${key}\\}`, 'g'), String(value));
    defJson = defJson.replace(new RegExp(`\\{\\{${key}\\}\\}`, 'g'), String(value));
  }

  const result = await safeQuery(
    `INSERT INTO "${schema}".workflows (name, definition, status, created_by)
     VALUES ($1, $2, 'active', $3) RETURNING *`,
    [params.name || templateName, defJson, createdBy]
  );
...
export async function startModuleWorkflow(
  ...
  if (!isCanonicalModuleCode(moduleCode)) {
    throw new Error(`'${moduleCode}' is not a canonical module code`);
  }

  const entry = MODULE_WORKFLOW_MAP[moduleCode as CanonicalModuleCode];
  if (!entry.primaryTemplateCode) {
    throw new WorkflowTemplateNotFoundError(moduleCode, '(null — platform-only module)');
  }

  // Verify the template exists in the library
  const libraryEntry = getWorkflowTemplateByCode(entry.primaryTemplateCode);
  if (!libraryEntry) {
    throw new WorkflowTemplateNotFoundError(moduleCode, entry.primaryTemplateCode);
  }

  // Instantiate using the existing resolution chain (in-memory → DB → workflows table)
  return instantiateTemplate(tenantId, entry.primaryTemplateCode, params, createdBy);
}
