...
import type { TriageProposal } from '../../../../types/cooperative-workflows.types';
...
export async function resolveTriageProposal(tenantId: string, proposalId: string, decision: {
  ...
  const propRes = await safeQuery(
    `SELECT * FROM "${schema}".triage_proposals WHERE proposal_id = $1`, [proposalId],
  );
  if (!propRes.rows.length) throw new Error('Proposal not found');
  const prop = getFirstRow(propRes);

  const assigneeId = decision.action === 'reassigned' && decision.newAssigneeId
    ? decision.newAssigneeId : prop.proposed_assignee_id;
  ...
  await safeQuery(
    `UPDATE "${schema}".triage_proposals
     ...
     WHERE proposal_id = $3`,
    [decision.action, decision.resolvedBy, proposalId],
  );

  // If accepted or reassigned, assign the task
  if (decision.action !== 'rejected') {
    await safeQuery(
      `UPDATE "${schema}".remediation_tasks SET assigned_to = $1 WHERE task_id = $2`,
      [assigneeId, prop.task_id],
    );
  }

  await recordAudit({
    tenantId, userId: decision.resolvedBy, module: 'cooperative-workflows',
    action: 'update', entityType: 'triage_proposal', entityId: proposalId,
    afterState: { decision: decision.action, assigneeId },
  });
}
...
function pickBestAssignee(task: any, members: any[]): {
...
function mapProposal(r: any): TriageProposal {
  return {
