export const NOTIFICATION_AI_CONFIG = {
  moduleCode: 'notification' as const,
  enabled: true,

  allowedActions: [
    'notification.alert.draft',
    'notification.alert.recommend',
    'notification.alert.summarize',
    'notification.alert.classify',
    'notification.alert.score',
    'notification.alert.analyze',
    'notification.alert.generate_report',
  ],

  blockedActions: [
    'notification.alert.delete',
    'notification.alert.approve',
    'notification.alert.reject',
    'notification.alert.override',
    'notification.alert.bulk_delete',
  ],

  humanInLoopBoundaries: {
    requiresHumanApproval: ['notification.alert.approve', 'notification.alert.reject', 'notification.alert.delete'],
    requiresHumanReview: ['notification.alert.classify', 'notification.alert.score'],
    autoExecutable: ['notification.alert.draft', 'notification.alert.summarize', 'notification.alert.analyze'],
  },
  ...
  promptContracts: {
    ...
    temperature: 0.3,
    systemPromptTemplate: 'notification_system_prompt_v1',
  },
...
export function isNotificationAiActionAllowed(action: string): boolean {
  return (NOTIFICATION_AI_CONFIG.allowedActions as readonly string[]).includes(action);
}

export function isNotificationAiActionBlocked(action: string): boolean {
  return (NOTIFICATION_AI_CONFIG.blockedActions as readonly string[]).includes(action);
}

export function requiresNotificationHumanApproval(action: string): boolean {
  return (NOTIFICATION_AI_CONFIG.humanInLoopBoundaries.requiresHumanApproval as readonly string[]).includes(action);
}
...
export async function summarize(tenantId: string, entityId: string): Promise<{ summary: string; keyPoints: string[]; confidence: number }> {
  ...
  const result = await safeQuery(
    `SELECT * FROM "${schema}".notifications WHERE id = $1`,
    [entityId]
  );
  const r = result.rows[0];
  if (!r) return { summary: 'Notification entity not found.', keyPoints: [], confidence: 0 };
  return {
    summary: `\${r.title ?? "Notification"}. Channel: \${r.channel ?? "in_app"}. Status: \${r.delivery_status ?? "N/A"}.`,
    keyPoints: [
      `Status: ${r.status ?? 'N/A'}`,
...
export async function generateReport(tenantId: string, filters?: { status?: string; dateFrom?: string; dateTo?: string }): Promise<{ totalCount: number; byStatus: Record<string, number>; insights: string[]; generatedAt: string }> {
  ...
  const countResult = await safeQuery(`SELECT COUNT(*) as total FROM "${schema}".notifications ${where}`, params);
  const statusResult = await safeQuery(`SELECT status, COUNT(*) as cnt FROM "${schema}".notifications ${where} GROUP BY status`, params);
