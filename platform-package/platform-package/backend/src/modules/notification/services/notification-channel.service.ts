...
function mapChannel(r: any): ChannelConfig {
  return {
    ...
    rateLimitPerHour: r.rate_limit_per_hour ? parseInt(r.rate_limit_per_hour, 10) : null,
...
function mapDelivery(r: any): DeliveryRecord {
  return {
...
export async function getChannelHealthSummaries(
  ...
  try {
    ...
    for (const r of deliveryResult.rows) {
    ...
    return configResult.rows.map((r: any) => {
      const stats = deliveryMap[r.channel_type] || { totalSent: 0, totalFailed: 0 };
      return {
