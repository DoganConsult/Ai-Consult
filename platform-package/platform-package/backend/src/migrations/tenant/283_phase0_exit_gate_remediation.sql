...
DO $$ BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = current_schema() AND table_name = 'ai_autonomy_state'
  ) AND NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'fk_aas_policy'
  ) THEN
...
-- Note: We don't add a hard FK here because enterprise_user_role_assignments
-- may not use UUID PKs consistently. Instead we add an index for join performance.
DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = current_schema() AND table_name = 'assignment_resolution_log'
  ) THEN
...
DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = current_schema() AND table_name = 'workflow_decision_log'
  ) THEN
...
DO $$ BEGIN
  IF EXISTS (
    ...
    SELECT 1 FROM pg_constraint WHERE conname = 'fk_wdl_transition'
  ) THEN
...
--     Drop old constraint, add new one with all values
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'ai_action_results') THEN
    ALTER TABLE ai_action_results ADD COLUMN IF NOT EXISTS policy_decision_id UUID;
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    SELECT 1 FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'policy_decision_log'
  ) AND EXISTS (
    SELECT 1 FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'ai_action_results'
  ) THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'ai_action_queue') THEN
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema = current_schema() AND table_name = 'v_user_effective_permissions') THEN
    COMMENT ON VIEW v_user_effective_permissions IS
      'DEPRECATED: Use effective_user_permissions directly. This view exists for backward compatibility during migration from legacy role_profiles.';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema = current_schema() AND table_name = 'v_user_visible_modules') THEN
    COMMENT ON VIEW v_user_visible_modules IS
      'DEPRECATED: Use effective_user_modules + deriveVisibleModules(). This view exists for backward compatibility during migration from hardcoded module lists.';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema = current_schema() AND table_name = 'v_module_catalog') THEN
    COMMENT ON VIEW v_module_catalog IS
      'DEPRECATED: Use product_modules + module_health_status + module_activation_status directly. This view exists for backward compatibility during migration from hardcoded MODULE_DESCRIPTIONS.';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema = current_schema() AND table_name = 'v_role_bundle_permissions') THEN
    COMMENT ON VIEW v_role_bundle_permissions IS
      'DEPRECATED: Use functional_role_bundles → functional_role_bundle_items → role_permissions chain. This view exists for backward compatibility during migration.';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema = current_schema() AND table_name = 'v_archetype_policy_summary') THEN
    COMMENT ON VIEW v_archetype_policy_summary IS
      'DEPRECATED: Use module_activation_policies + module_workflow_profiles + ai_action_policies directly. This view exists for backward compatibility.';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema = current_schema() AND table_name = 'v_workflow_state_machine') THEN
    COMMENT ON VIEW v_workflow_state_machine IS
      'DEPRECATED: Use workflow_profile_states + workflow_profile_transitions with workflow-profile-resolver.service.ts. This view exists for backward compatibility.';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema = current_schema() AND table_name = 'v_ai_autonomy_resolved') THEN
    COMMENT ON VIEW v_ai_autonomy_resolved IS
      'DEPRECATED: Use ai_autonomy_state directly. This view exists for backward compatibility during migration.';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema = current_schema() AND table_name = 'v_approver_chain') THEN
    COMMENT ON VIEW v_approver_chain IS
      'DEPRECATED: Use approver_resolution_cache directly with user-assignment.service.ts. This view exists for backward compatibility.';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema = current_schema() AND table_name = 'v_active_overrides') THEN
    COMMENT ON VIEW v_active_overrides IS
      'DEPRECATED: Use runtime_overrides with runtime-overrides.service.ts. This view exists for backward compatibility.';
  END IF;
END $$;
