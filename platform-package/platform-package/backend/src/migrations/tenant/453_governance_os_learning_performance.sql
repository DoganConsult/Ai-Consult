...
CREATE INDEX IF NOT EXISTS idx_learning_scores_trend 
  ON os_learning_scores(tenant_id, trend, overall_learning_score) 
...
CREATE INDEX IF NOT EXISTS idx_knowledge_articles_store_published 
  ON os_knowledge_articles(tenant_id, knowledge_store, status, created_at DESC);
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = current_schema AND table_name = 'os_knowledge_articles' AND column_name = 'tags') THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_knowledge_articles_tags ON os_knowledge_articles USING GIN(tags) WHERE status = ''published''';
  END IF;
END $$;
...
SELECT 
  ...
  scope_type,
  scope_key,
  AVG(overall_learning_score) as avg_score,
  MAX(overall_learning_score) as max_score,
  MIN(overall_learning_score) as min_score,
