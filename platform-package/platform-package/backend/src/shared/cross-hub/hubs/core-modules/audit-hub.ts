...
import {
  ...
  enterpriseCreateTask, safeCreateTask, _safeCreateActionItem,
  ...
  sub('audit.finding_created', 'xhub-audit→risk-register', async (e) => {
    ...
    try {
      ...
      await createRemediationTask(tenantId, {
        ...
        assigned_to: admin,
        priority: priorityMap[String(severity)] || 'medium',
        due_date: daysFromNow(dueDaysMap[String(severity)] || 21),
      });
