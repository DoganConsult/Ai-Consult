...
import { mapConnectorOutputToEvidence } from '../../../../modules/integrations/services/connector-evidence-mapper.service';
...
export function registerCcmConnectorHub(sub: SubFn): void {
  ...
  sub('connector.sync_completed', 'xhub-connector→evidence-collect', async (e) => {
    ...
    await mapConnectorOutputToEvidence(tenantId, {
      connectorType: payload?.connectorType ?? payload?.connectorName ?? (e as Record<string, any>).entityType as string ?? 'connector',
