...
const createSnapshot = (overrides?: Partial<AccessSnapshot>): AccessSnapshot => ({
  identity: { userId: 'u-001', email: 'admin@acme.com', displayName: 'Admin', actorType: 'human' },
  tenant: { tenantId: 't-1', status: 'active', plan: 'enterprise' },
  membership: { status: 'active', membershipType: 'owner', joinedAt: '2026-01-01' },
  accessProfiles: ['platform_admin'],
...
describe('FrontendAccessContract — getLandingPage', () => {
  ...
  it('returns custom landing page', () => {
    const snapshot = createSnapshot({ landingPage: '/risk/overview' });
