...
export async function getUserAuthorityLevel(tenantId: string, userId: string): Promise<{ levelCode: string; rank: number } | null> {
  ...
  return result.rows[0] ? { levelCode: result.rows[0].level_code, rank: result.rows[0].rank } : null;
}
...
export async function hasAuthority(tenantId: string, userId: string, requiredLevel: string): Promise<boolean> {
  ...
  return user.rank >= (required.rows[0]?.rank ?? 999);
}
...
export async function getAuthorityChain(
  ...
  return result.rows.map((r: any) => ({ levelCode: r.level_code, rank: r.rank }));
}
...
export async function getAuthorityLevels(
  ...
  return result.rows.map((r: any) => ({ levelCode: r.level_code, rank: r.rank, label: r.label ?? null }));
}
...
export async function resolveApprovalChain(
  ...
  return result.rows.map((r: any) => ({ userId: r.user_id, levelCode: r.level_code, rank: r.rank }));
}
