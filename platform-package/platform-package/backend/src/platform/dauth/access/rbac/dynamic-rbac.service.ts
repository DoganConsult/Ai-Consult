...
/** @deprecated Fallback — runtime loads from module_role_bindings in DB. */
const MODULE_ROLE_MAP_FALLBACK: Record<string, string[]> = {
...
const moduleRoleCache = new Map<string, { map: Record<string, string[]>; ts: number }>();
const MR_CACHE_TTL = 120_000;

async function __getModuleRoleMap(tenantId: string): Promise<Record<string, string[]>> {
  const cached = moduleRoleCache.get(tenantId);
  if (cached && Date.now() - cached.ts < MR_CACHE_TTL) return cached.map;
  try {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(
      `SELECT fr.module_code, fr.role_code FROM "${schema}".functional_roles fr WHERE fr.module_code IS NOT NULL`,
    );
    const map: Record<string, string[]> = {};
    for (const r of rows) {
      if (!map[r.module_code]) map[r.module_code] = [];
      map[r.module_code].push(r.role_code);
    }
    moduleRoleCache.set(tenantId, { map, ts: Date.now() });
    return map;
  } catch { return MODULE_ROLE_MAP_FALLBACK; }
}

const MODULE_ROLE_MAP = MODULE_ROLE_MAP_FALLBACK;
