...
import { getEffectiveModules } from '../../dos/modules/effective-modules.service';
...
export async function resolveAccessSnapshot(
  ...
  try {
    ...
    const row = getFirstRow(memberResult);
    if (row) {
  ...
  try {
    ...
    productEntitlements = entResult.rows.map((r: Record<string, unknown>) => ({
  ...
  try {
    ...
    const payload = raw as any as _AuthzPayload;
  ...
  const visibleProducts = productEntitlements.length > 0
    ...
    : [process.env.DEFAULT_PRODUCT_CODE ?? 'default']; // @ownership-note: product code resolved from env, not hardcoded product reference
