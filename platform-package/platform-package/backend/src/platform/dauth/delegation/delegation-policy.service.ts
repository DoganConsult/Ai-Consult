...
export async function getDelegationPolicies(tenantId: string): Promise<DelegationPolicy[]> {
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `SELECT policy_id, role_code, max_duration_hours, allowed_scopes,
...
export async function getDelegationPolicyForRole(
  tenantId: string,
  roleCode: string,
): Promise<DelegationPolicy | null> {
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `SELECT policy_id, role_code, max_duration_hours, allowed_scopes,
            allowed_actions, requires_approval, is_active
     FROM "${schema}".delegation_policies
     WHERE role_code = $1 AND is_active = TRUE LIMIT 1`,
    [roleCode],
  );
...
export async function validateDelegationRequest(
  tenantId: string,
  delegatorRoles: string[],
  requestedScopes: string[],
  requestedDurationHours: number,
): Promise<{ valid: boolean; reason: string }> {
  const secPolicy = await getTenantSecurityPolicy(tenantId);
...
// ── Delegation Rules (migrated from delegation-rules.service.ts) ──────────
// Shadow agent delegation rules: per-user per-agent action-type rules with
// risk-level, time-window, and daily-limit enforcement.
...
export async function evaluateDelegation(
  ...
  try {
    const shadowResult = await safeQuery(
      `SELECT enabled, consent_granted, autonomy_level, max_actions_per_day,
    ...
    const resetDate = cfg.actions_today_reset?.toISOString?.().split('T')[0] || today;
    ...
    const ruleResult = await safeQuery(
      `SELECT * FROM "${schema}".delegation_rules
       WHERE tenant_id = $1 AND user_id = $2 AND agent_id = $3 AND action_type = $4`,
      [tenantId, userId, agentId, actionType],
    );

    if (ruleResult.rows.length > 0) {
      const rule: DelegationRule = getFirstRow(ruleResult);
    ...
    logger.warn(`[DelegationRules] evaluateDelegation failed: ${toErrorMessage(err)}`);
    return { allowed: false, reason: `Delegation check error: ${toErrorMessage(err)}`, requiresNotification: true };
  }
}

export async function incrementDailyActions(tenantId: string, userId: string): Promise<void> {
  const schema = tenantSchema(tenantId);
  const today = new Date().toISOString().split('T')[0];
  try {
    await safeQuery(
...
export async function upsertDelegationRule(
  ...
  try {
    const result = await safeQuery(
      `INSERT INTO "${schema}".delegation_rules
    ...
    return getFirstRow(result)?.rule_id || null;
  } catch (err: unknown) {
    logger.warn(`[DelegationRules] upsertDelegationRule failed: ${toErrorMessage(err)}`);
    return null;
  }
}

export async function deleteDelegationRule(tenantId: string, ruleId: string): Promise<boolean> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
      `DELETE FROM "${schema}".delegation_rules WHERE rule_id = $1 AND tenant_id = $2 RETURNING rule_id`,
      [ruleId, tenantId],
    );
