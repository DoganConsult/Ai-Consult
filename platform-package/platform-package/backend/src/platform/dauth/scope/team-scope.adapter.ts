...
export async function resolveTeamScope(
  ...
  return rows.map((r: any) => r.scope_id);
}
...
export async function resolveTeamMembership(
  ...
  return rows.map((r: any) => ({ teamId: r.team_id, teamRole: r.team_role }));
}
...
export async function getTeamMembers(
  ...
  return rows.map((r: any) => ({ userId: r.user_id, teamRole: r.team_role }));
}
