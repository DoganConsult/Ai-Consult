  ...
  if (enrichments.includes('roles')) {
    queries.push(
      safeQuery(
        ...
        enriched.roles = result.rows.map((r: any) => r.role_code).filter(Boolean);
  ...
  if (enrichments.includes('permissions')) {
    queries.push(
      safeQuery(
        ...
        enriched.permissions = result.rows.map((r: any) => r.permission_code).filter(Boolean);
  ...
  if (enrichments.includes('scopes')) {
    queries.push(
      safeQuery(
        ...
        enriched.scopes = result.rows.map((r: any) => ({
  ...
  if (enrichments.includes('authorities')) {
    queries.push(
      safeQuery(
        ...
        enriched.authorities = result.rows.map((r: any) => r.authority_code).filter(Boolean);
  ...
  if (enrichments.includes('accessProfiles')) {
    queries.push(
      safeQuery(
        ...
        enriched.accessProfiles = result.rows.map((r: any) => r.access_profile_code).filter(Boolean);
...
export async function getPrincipalContext(
  ...
  const memberships = membershipsResult.rows.map((r: any) => ({
  ...
  const accessProfiles = profilesResult.rows
    .map((r: any) => r.access_profile_code)
    .filter(Boolean);
