...
export async function verifyEmailChallenge(userId: string, code: string): Promise<boolean | { error: string }> {
  // Brute force protection: deny if > 5 failed MFA attempts in last 10 minutes
  const recentFailures = await safeQuery(
    `SELECT COUNT(*)::int AS cnt FROM login_attempts
     WHERE user_id = $1 AND success = FALSE AND attempt_type = 'mfa'
       AND attempted_at > NOW() - INTERVAL '10 minutes'`,
    [userId],
  );
  const failCount = recentFailures.rows[0]?.cnt ?? 0;
  if (failCount > 5) {
    return { error: 'Too many MFA attempts. Try again later.' };
  }
  ...
  if (!result.rows.length) {
    ...
    await safeQuery(
      `INSERT INTO login_attempts (user_id, success, attempt_type, attempted_at)
       VALUES ($1, FALSE, 'mfa', NOW())`,
      [userId],
    ).catch(() => {});
    return false;
  }
...
export async function enableTotp(
  ...
  const userRow = await query('SELECT email FROM users WHERE user_id = $1 LIMIT 1', [userId]);
  const email = userRow.rows[0]?.email || userId;
  const issuer = process.env.MFA_ISSUER_NAME || process.env.PRODUCT_NAME || 'Dogan-AI-OS';
  const otpauthUrl = otplib.generateURI({ issuer, label: email, secret });
  await query(
    `INSERT INTO user_mfa (user_id, mfa_type, secret, enabled)
  ...
  try {
    const { generateQRCodeDataURL } = await import('../../dos/services/document-generation/qrcode.service');
