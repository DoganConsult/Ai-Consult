...
vi.mock('./token-blacklist.service', () => ({
  ...
  revokeAllUserTokens: (...args: any[]) => mockRevokeAllUserTokens(...args),
}));
...
describe('DAuth SessionService', () => {
  describe('createSession', () => {
    ...
    it('passes optional fields to payload', async () => {
      const result = await createSession({
        userId: 'u-1',
        email: 'a@b.com',
        tenantId: 't-1',
  ...
  describe('refreshSession', () => {
    ...
    it('returns null when refresh token is invalid', async () => {
      vi.mocked(verifyRefreshToken).mockReturnValue(null as any);
