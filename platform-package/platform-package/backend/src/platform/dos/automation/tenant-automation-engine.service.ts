 ...
 *
 * Platform-owned reusable automation engine.
 * Product-specific workers register via registerAutomationWorker().
 *
 * @owner DOS
 * @since 2026-03-30
 */
...
export interface AutomationWorker {
  ...
  /** Owner code (e.g., 'dos', 'risk', 'compliance') */
  ownerCode: string;
...
export class TenantAutomationEngine {
  ...
  async listActiveTenants(): Promise<string[]> {
    const result = await query(`SELECT tenant_id FROM public.tenants WHERE status = 'active'`);
    return result.rows.map((r: Record<string, any>) => r.tenant_id);
  }
}
