...
export async function approveStep(
  ...
  if (requestValue !== undefined && requestValue > 0) {
    try {
      const { checkActorAuthority } = await import('../../../../platform/dauth/authority/approval-matrix.service');
  ...
  if (isLastStep) {
    ...
    try {
      const { emitWorkflowEvent } = await import('../events/workflow-event-emitter.service');
      emitWorkflowEvent({
    ...
    try {
      const { onApprovalResolved } = await import('../engine/workflow-engine.service');
...
function mapChainRow(row: any): ApprovalChainRecord {
...
function mapRequestRow(row: any): ApprovalRequestRecord {
  return {
