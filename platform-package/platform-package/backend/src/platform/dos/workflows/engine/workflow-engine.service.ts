...
import { swallowNull, EC , catchHandler } from '../../../../utils/resilient-catch';

async function resolveAssigneeToUserId(_tenantId: string, opts: Record<string, unknown>): Promise<string | null> {
...
function entityContextOptions(context: Record<string, unknown>): Record<string, unknown> {
...
export interface EngineStep {
  ...
  auto_assign_rule: Record<string, any> | null;
}
...
async function loadSlaPolicies(schema: string, definitionId: string, stepId: string): Promise<Array<{ warning_hours: number | null; breach_hours: number | null; escalation_action: Record<string, any> | null }>> {
  const result = await safeQuery(
...
export function evaluateCondition(
  ...
  const expr = condition.expression as Record<string, any> | null;
  if (!expr) return true;
...
function __findEndSteps(steps: EngineStep[]): EngineStep[] {
...
async function resolveAssignee(
  ...
  if (step.auto_assign_rule) {
    const rule = step.auto_assign_rule as Record<string, any>;
...
export async function startWorkflowExecution(
  ...
  const __transitions = await loadTransitions(schema, definitionId);
  ...
  const instanceId = getFirstRow(execResult)?.execution_id;
  ...
  if (startStep.is_end) {
    ...
    await safeQuery(
      `UPDATE "${schema}".workflow_executions SET status = 'completed', completed_at = NOW() WHERE execution_id = $1`,
      [instanceId],
    );
    await emitWorkflowEvent({
      tenantId,
...
export async function advanceStep(
  ...
  try {
  const execResult = await safeQuery(
    `SELECT workflow_id, status FROM "${schema}".workflow_executions WHERE execution_id = $1`,
    [instanceId],
  );
  ...
  if (activeStep.rows.length > 0) {
    await completeInstanceStep(
      ...
      transitionData as Record<string, any>,
      userId,
    );
  }
  ...
  return advanceResult;
    ...
    await safeQuery(`SELECT pg_advisory_unlock($1)`, [lockId]).catch(() => {});
  }
}
...
export async function completeStep(
  ...
  const execResult = await safeQuery(
    `SELECT workflow_id FROM "${schema}".workflow_executions WHERE execution_id = $1`,
    [instanceId],
  );
...
export async function cancelExecution(
  ...
  const exec = await safeQuery(
    `SELECT status FROM "${schema}".workflow_executions WHERE execution_id = $1`,
    [instanceId],
  );
...
export async function getInstanceStatus(
  ...
  const exec = await safeQuery(
    `SELECT execution_id, workflow_id, status FROM "${schema}".workflow_executions WHERE execution_id = $1`,
    [instanceId],
  );
  if (exec.rows.length === 0) {
    throw new Error(`[WorkflowEngine] Instance ${instanceId} not found.`);
...
export async function createApprovalForStep(
  ...
  try {
    const { submitForApproval } = await import('../approvals/approval-engine.service');
    ...
    await safeQuery(
      `UPDATE "${tenantSchema(tenantId)}".workflow_executions
       SET status = 'paused' WHERE execution_id = $1`,
      [instanceId],
    );
...
export async function onApprovalResolved(
  ...
  if (decision === 'approved') {
    await safeQuery(
      `UPDATE "${tenantSchema(resolvedTenantId)}".workflow_executions
       SET status = 'running' WHERE execution_id = $1`,
      [instanceId],
    );
    ...
    await safeQuery(
      `UPDATE "${schema}".workflow_executions SET status = 'failed', completed_at = NOW() WHERE execution_id = $1`,
      [instanceId],
    );
...
export async function getFailurePath(
  ...
  instanceId: string,
  currentStepId: string,
): Promise<{ stepId: string; transitionId: string; conditions: Record<string, any> } | null> {
  const schema = tenantSchema(tenantId);
