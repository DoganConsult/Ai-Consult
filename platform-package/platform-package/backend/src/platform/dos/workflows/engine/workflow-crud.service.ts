// ============================================
// Shahin GRC — Workflow CRUD + Serialization
// Create, read, update workflows; serialize/
...
export async function saveWorkflow(tenantId: string, data: {
  ...
  if (data.workflowId) {
    ...
    const existing = await safeQuery(
      `SELECT version FROM "${schema}".workflows WHERE workflow_id = $1`, [data.workflowId]
    );
  ...
  // Create new
  const result = await safeQuery(
    `INSERT INTO "${schema}".workflows (name, definition, created_by, department_id)
     VALUES ($1, $2, $3, $4)
     RETURNING *`,
...
export async function getWorkflows(tenantId: string, options?: { departmentId?: string | null }): Promise<WorkflowRow[]> {
  const schema = tenantSchema(tenantId);
  let sql = `SELECT * FROM "${schema}".workflows
     WHERE definition->>'type' IS DISTINCT FROM 'audit_plan'`;
  const params: (string | null)[] = [];
  if (options?.departmentId !== undefined) {
    // Show tenant-wide (department_id IS NULL) or workflows scoped to the given department
...
export async function getWorkflowById(tenantId: string, workflowId: string): Promise<WorkflowRow | null> {
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT * FROM "${schema}".workflows WHERE workflow_id = $1`, [workflowId]
  );
...
export async function getExecutionDetail(tenantId: string, executionId: string): Promise<{
  ...
  const execRes = await safeQuery(
    `SELECT we.execution_id, we.workflow_id, we.trigger_type, we.status, we.started_at, we.completed_at,
...
export async function getExecutionActivity(tenantId: string, executionId: string): Promise<{ activities: WorkflowStep[] }> {
  ...
  const res = await safeQuery(
    `SELECT step_log FROM "${schema}".workflow_instances WHERE execution_id = $1`, [executionId]
  );
