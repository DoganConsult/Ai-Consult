...
export async function getTaskTypePermissionAction(tenantId: string, taskType: string): Promise<string> {
  if (!_taskTypeCache || _taskTypeCache.expiresAt <= Date.now()) {
    try {
      ...
      const res = await safeQuery(`SELECT task_type, permission_action FROM "${schema}".task_type_config`);
      const map = new Map<string, string>();
      for (const r of res.rows) map.set(r.task_type, r.permission_action);
