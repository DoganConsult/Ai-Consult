...
export const logger = {
  info: (message: string, ...args: unknown[]) => {
    ...
    logger.info(formatEntry('info', message, extra!), ...(extra ? [] : args));
  },
  error: (message: string, ...args: unknown[]) => {
    if (!shouldLog('error')) return;
    const extra = args.length === 1 && typeof args[0] === 'object' ? args[0] : undefined;
    logger.error(formatEntry('error', message, extra!), ...(extra ? [] : args));
  },
  warn: (message: string, ...args: unknown[]) => {
    if (!shouldLog('warn')) return;
    const extra = args.length === 1 && typeof args[0] === 'object' ? args[0] : undefined;
    logger.warn(formatEntry('warn', message, extra!), ...(extra ? [] : args));
  },
  debug: (message: string, ...args: unknown[]) => {
    if (!shouldLog('debug')) return;
    const extra = args.length === 1 && typeof args[0] === 'object' ? args[0] : undefined;
    logger.info(formatEntry('debug', message, extra!), ...(extra ? [] : args));
  },
};
