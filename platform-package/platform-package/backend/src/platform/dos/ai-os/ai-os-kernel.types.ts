...
// ── DB Row Interfaces (replace `(r: any)` casts in service) ─────────────────
...
// ── Kernel Log Event (typed return for getKernelLog) ────────────────────────
...
// ── Response View Models (explicit response shapes) ─────────────────────────

export interface KernelProcessView {
  processes: AiProcess[];
  summary: { running: number; queued: number; blocked: number; total: number };
}

export interface KernelSchedulerView {
  entries: SchedulerEntry[];
  summary: { total: number; active: number; backpressure: number };
}
...
export interface KernelMemoryView {
  ...
  summary: { agents: number; totalMemories: number; totalTokens: number; staleMemories: number };
}
...
export interface KernelOverviewView {
  ...
  processes: {
    recent: AiProcess[];
    summary: { running: number; queued: number; failed24h: number; total24h: number };
  };
  scheduler: {
    entries: SchedulerEntry[];
    active: number;
