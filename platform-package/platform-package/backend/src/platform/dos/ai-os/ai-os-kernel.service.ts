...
import { emptyResult, safeQuery, tenantSchema, query as dbQuery } from '../../../config/database/database';
...
export interface AiProcess {
  ...
  memoryUsed: number;    // tokens_used from agent_runs (migration 700)
  cpuTime: number;       // duration_ms from agent_runs
  parentPid: string | null; // parent_run_id from agent_runs (migration 700)
...
export interface KernelStatus {
  uptime: string;
  bootedAt: string;
  processCount: { running: number; queued: number; blocked: number; total: number };
  agentCount: { enabled: number; disabled: number; circuitOpen: number };
...
export async function getProcessTable(tenantId: string): Promise<AiProcess[]> {
  ...
  try {
    ...
    logger.warn('[Kernel] getProcessTable failed', { error: toErrorMessage(err) });
    return [];
  }
}
...
export async function getKernelStatus(tenantId: string): Promise<KernelStatus> {
  const schema = tenantSchema(tenantId);
  ...
  const [processRes, agentRes, circuitRes, latencyRes, ipcRes] = await Promise.all([
    ...
    swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ running: 0, queued: 0, blocked: 0, total: 0 }]), safeQuery(`
      SELECT
    ...
    swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ enabled: 0, disabled: 0 }]), safeQuery(`
      SELECT
    ...
    swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ circuit_open: 0 }]), safeQuery(`
    ...
    swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ avg_latency: 0, total_tokens: 0, error_rate: 0 }]), safeQuery(`
      SELECT
    ...
    swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ pending: 0 }]), safeQuery(`
  ...
  try {
    const budgetRes = await safeQuery(
      `SELECT monthly_token_budget FROM "${schema}".tenant_ai_config LIMIT 1`
    );
  ...
  const tokenUtilization = Math.round(
    (tokensUsed24h / Math.max(tokenBudget / 30, 1)) * 100
  );

  const schedulerLoad = Math.min(100, Math.round(
    (Number(proc.running) * 10 + Number(proc.queued) * 5) /
    Math.max(Number(agent.enabled), 1)
  ));

  const memoryPressure: KernelStatus['memoryPressure'] =
    tokenUtilization > 90 ? 'critical' :
  ...
  try {
    const bootRes = await safeQuery(
      `SELECT MIN(updated_at) AS booted FROM "${schema}".agent_runtime_config`
    );
    if (bootRes.rows[0]?.booted) bootedAt = new Date(bootRes.rows[0].booted).toISOString();
  } catch { /* use now */ }
...
export async function getSchedulerTable(tenantId: string): Promise<SchedulerEntry[]> {
  const schema = tenantSchema(tenantId);
  try {
    // Read real scheduler entries from job_registry (public schema)
    const jobRes = await dbQuery(
      `SELECT job_name, cron_expression, enabled, last_run_at, last_status, next_run_at
    ...
    const agentRes = await safeQuery(
      `SELECT agent_id, enabled FROM "${schema}".agent_runtime_config`
    );
    const agentEnabled = new Map<string, boolean>();
    for (const r of agentRes.rows) {
      agentEnabled.set(String(r.agent_id), r.enabled !== false);
    }

    return jobRes.rows.map((r: Record<string, any>, i: number) => {
      ...
      return {
        ...
        lastResult: mapLastResult(r.last_status as string | null),
        priority: i,
    ...
    logger.warn('[Kernel] getSchedulerTable failed', { error: toErrorMessage(err) });
    return [];
  }
}
...
export async function getIpcMessages(tenantId: string, limit = 50): Promise<IpcMessage[]> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(`
      SELECT
    ...
    logger.warn('[Kernel] getIpcMessages failed', { error: toErrorMessage(err) });
    return [];
  }
}
...
export async function getMemoryPartitions(tenantId: string): Promise<MemoryPartition[]> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(`
      SELECT
    ...
    logger.warn('[Kernel] getMemoryPartitions failed', { error: toErrorMessage(err) });
    return [];
  }
}
...
export async function getKernelLog(tenantId: string, limit = 100): Promise<KernelLogEvent[]> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(`
      (
    ...
    return result.rows.map((r: Record<string, any>): KernelLogEvent => ({
      timestamp: toISOOrNull(r.created_at) || new Date().toISOString(),
      source: (r.source as KernelLogEvent['source']) || 'agent_run',
    ...
    logger.warn('[Kernel] getKernelLog failed', { error: toErrorMessage(err) });
    return [];
  }
}
...
export async function killProcess(tenantId: string, runId: string): Promise<boolean> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(`
    ...
    logger.warn('[Kernel] killProcess failed', { runId, error: toErrorMessage(err) });
    return false;
  }
}
...
export async function rebootAgent(
  tenantId: string,
  agentId: string,
): Promise<{ reset: boolean; circuitCleared: boolean }> {
  const schema = tenantSchema(tenantId);
  let reset = false;
  let circuitCleared = false;

  try {
    // Clear circuit breaker (in-memory; no DB columns for circuit state)
    try {
      const { resetAgentCircuit } = await import('../../../modules/ai/services/orchestration/ai-agent-runtime.service');
      resetAgentCircuit(tenantId, agentId);
      circuitCleared = true;
    } catch {
      // per-agent-circuit-breaker may not be loaded; non-fatal
    ...
    logger.warn('[Kernel] rebootAgent partial failure', {
      agentId, error: toErrorMessage(err),
    });
  }
...
function mapRunStatus(s: string): AiProcess['state'] {
  switch (s) {
...
function mapPriority(mode: string): AiProcess['priority'] {
  switch (mode) {
...
function mapLastResult(s: string | null): SchedulerEntry['lastResult'] {
  if (!s) return null;
  switch (s) {
...
function safeParse(payload: any): Record<string, any> {
  if (!payload) return {};
 ...
 */
export async function adjustAutonomyLevel(
  tenantId: string, agentId: string, level: string,
): Promise<{ success: boolean; previousLevel: string; newLevel: string }> {
  const s = tenantSchema(tenantId);
  const prev = await safeQuery(
    `SELECT autonomy_level FROM "${s}".agent_runtime_config WHERE agent_id = $1`, [agentId],
  );
  const previousLevel = String(prev.rows[0]?.autonomy_level || 'hybrid');
  await safeQuery(
    `UPDATE "${s}".agent_runtime_config SET autonomy_level = $2, updated_at = NOW() WHERE agent_id = $1`,
    [agentId, level],
  );
 ...
 */
export async function injectPriority(
  tenantId: string, agentId: string,
  directive: { focus: string; urgency: 'critical' | 'high' | 'medium' | 'low'; context?: string },
): Promise<{ success: boolean; message: string }> {
  const s = tenantSchema(tenantId);
  await safeQuery(
    `UPDATE "${s}".agent_runtime_config SET priority_directive = $2::jsonb, updated_at = NOW() WHERE agent_id = $1`,
    [agentId, JSON.stringify(directive)],
  );
 ...
 */
export async function killRunningAction(
  tenantId: string, runId: string,
): Promise<{ success: boolean; message: string }> {
  const s = tenantSchema(tenantId);
  await safeQuery(
    `UPDATE "${s}".agent_runs SET status = 'cancelled', updated_at = NOW() WHERE run_id = $1 AND status = 'running'`,
    [runId],
  );
...
export async function getTokenUsage(
  tenantId: string, windowHours: number = 24,
): Promise<{ totalTokens: number; byAgent: Record<string, number>; avgPerRun: number }> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT agent_id, COALESCE(SUM(tokens_used), 0)::int AS total, COUNT(*)::int AS runs
     FROM "${s}".agent_runs
 ...
 */
export async function setGlobalAutonomyLevel(
  tenantId: string, level: string,
): Promise<{ success: boolean; agentsUpdated: number }> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `UPDATE "${s}".agent_runtime_config SET autonomy_level = $1, updated_at = NOW()`, [level],
  );
 ...
 */
export async function getPriorityDirective(
  tenantId: string, agentId: string,
): Promise<{ focus: string; urgency: string; context?: string } | null> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT priority_directive FROM "${s}".agent_runtime_config WHERE agent_id = $1`, [agentId],
  );
 ...
 */
export async function clearPriorityDirective(tenantId: string, agentId: string): Promise<void> {
  const s = tenantSchema(tenantId);
  await safeQuery(
    `UPDATE "${s}".agent_runtime_config SET priority_directive = NULL, updated_at = NOW() WHERE agent_id = $1`,
    [agentId],
  );
}

/** Log a kernel command event to the tenant event log. */
async function logKernelEvent(tenantId: string, eventType: string, entityId: string, payload: any): Promise<void> {
  try {
    const s = tenantSchema(tenantId);
    await safeQuery(
      `INSERT INTO "${s}".agrc_event_log (event_type, entity_type, entity_id, payload, created_at)
       VALUES ($1, 'kernel', $2, $3::jsonb, NOW())`,
      [eventType, entityId, JSON.stringify(payload)],
    );
  } catch { /* non-critical logging */ }
}

// ── R1: Process Detail (/proc/:pid) ─────────────────────────────────────────
...
export async function getProcessDetail(tenantId: string, pid: string): Promise<ProcessDetail | null> {
  const schema = tenantSchema(tenantId);
  try {
    // 1. Get the run
    ...
    const r = runRes.rows[0] as Record<string, any>;
    ...
    `, [pid]);
    const steps: AgentStep[] = stepsRes.rows.map((s: Record<string, any>) => ({
    ...
    // 3. Get child runs
    const childRes = await safeQuery(`
      SELECT run_id AS pid, agent_id, status AS state, COALESCE(platform_mode,'hybrid') AS priority,
             created_at AS started_at,
             CASE WHEN status IN ('completed','failed','cancelled') THEN updated_at ELSE NULL END AS completed_at,
             COALESCE(duration_ms,0) AS duration_ms, COALESCE(tokens_used,0) AS memory_used,
    ...
    `, [pid]);
    const childProcesses: AiProcess[] = childRes.rows.map((c: Record<string, any>) => ({
    ...
    logger.warn('[Kernel] getProcessDetail failed', { pid, error: toErrorMessage(err) });
    return null;
  }
}

// ── R1: Agent Detail (/agent/:agentId) ──────────────────────────────────────

export interface AgentDetail {
  agentId: string;
  config: { enabled: boolean; pausedAt: string | null; maxRetries: number; cooldownSeconds: number };
  circuit: { state: string; failureCount: number; lastFailureAt: string | null };
  runs24h: { total: number; completed: number; failed: number; avgDurationMs: number };
...
export async function getAgentDetail(tenantId: string, agentId: string): Promise<AgentDetail | null> {
  ...
  try {
    const [configRes, circuitRes, runsRes, memRes, recentRes] = await Promise.all([
      safeQuery(`SELECT enabled, paused_at, max_retries, cooldown_seconds FROM "${schema}".agent_runtime_config WHERE agent_id = $1`, [agentId]),
      swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(`SELECT state, failure_count, last_failure_at FROM "${schema}".agent_circuit_breaker WHERE agent_id = $1`, [agentId]), { tenantId: tenantId, operation: 'query agent_runtime_config' }),
      safeQuery(`
    ...
    const cfg = configRes.rows[0] as Record<string, any> | undefined;
    if (!cfg) return null;

    const cb = (circuitRes.rows[0] || {}) as Record<string, any>;
    const runs = (runsRes.rows[0] || {}) as Record<string, any>;
    const mem = memRes.rows[0] as Record<string, any> | undefined;
    ...
    return {
      ...
      recentRuns: recentRes.rows.map((r: Record<string, any>) => ({
        pid: String(r.pid), agentId: String(r.agent_id), state: mapRunStatus(String(r.state || '')),
        priority: mapPriority(String(r.priority || '')), startedAt: toISOOrNull(r.started_at),
        completedAt: toISOOrNull(r.completed_at), durationMs: Number(r.duration_ms) || null,
        memoryUsed: Number(r.memory_used) || 0, cpuTime: Number(r.duration_ms) || 0,
    ...
    logger.warn('[Kernel] getAgentDetail failed', { agentId, error: toErrorMessage(err) });
    return null;
  }
}
...
export async function pauseAgent(tenantId: string, agentId: string): Promise<boolean> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
      `UPDATE "${schema}".agent_runtime_config SET paused_at = NOW(), updated_at = NOW() WHERE agent_id = $1 AND paused_at IS NULL`,
      [agentId],
    );
    return (result.rowCount ?? 0) > 0;
  } catch (err) {
    logger.warn('[Kernel] pauseAgent failed', { agentId, error: toErrorMessage(err) });
    return false;
  }
}

export async function resumeAgent(tenantId: string, agentId: string): Promise<boolean> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
      `UPDATE "${schema}".agent_runtime_config SET paused_at = NULL, updated_at = NOW() WHERE agent_id = $1 AND paused_at IS NOT NULL`,
      [agentId],
    );
    return (result.rowCount ?? 0) > 0;
  } catch (err) {
    logger.warn('[Kernel] resumeAgent failed', { agentId, error: toErrorMessage(err) });
    return false;
  }
}
...
export async function getKernelHealth(tenantId: string): Promise<KernelHealth> {
  const schema = tenantSchema(tenantId);
  const [agentRes, circuitRes, stuckRes, failedRes, ipcRes] = await Promise.all([
    swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ cnt: 0 }]), safeQuery(`SELECT COUNT(*) FILTER (WHERE enabled=true)::int AS cnt FROM "${schema}".agent_runtime_config`), { tenantId: tenantId, operation: 'query agent_runtime_config' }),
    swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ cnt: 0 }]), safeQuery(`SELECT COUNT(*)::int AS cnt FROM "${schema}".agent_circuit_breaker WHERE state='open'`), { tenantId: tenantId, operation: 'query agent_runtime_config' }),
    swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ cnt: 0 }]), safeQuery(`SELECT COUNT(*)::int AS cnt FROM "${schema}".agent_runs WHERE status='running' AND created_at < NOW()-INTERVAL '10 minutes'`), { tenantId: tenantId, operation: 'query agent_runtime_config' }),
    swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ cnt: 0 }]), safeQuery(`SELECT COUNT(*)::int AS cnt FROM "${schema}".agent_runs WHERE status='failed' AND created_at > NOW()-INTERVAL '24 hours'`), { tenantId: tenantId, operation: 'query agent_circuit_breaker' }),
    swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ cnt: 0 }]), safeQuery(`SELECT COUNT(*)::int AS cnt FROM "${schema}".agent_handoffs WHERE status='pending'`), { tenantId: tenantId, operation: 'query agent_runs' }),
  ]);
...
export async function saveKernelSnapshot(tenantId: string): Promise<string> {
  const schema = tenantSchema(tenantId);
  ...
  const result = await safeQuery(
    `INSERT INTO "${schema}".kernel_snapshots (tenant_id, snapshot_data) VALUES ($1, $2) RETURNING snapshot_id`,
    [tenantId, JSON.stringify(snapshotData)],
  );
  return result.rows[0]?.snapshot_id || '';
}

export async function listKernelSnapshots(tenantId: string, limit = 20): Promise<Array<{ snapshotId: string; createdAt: string; data: Record<string, any> }>> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
      `SELECT snapshot_id, created_at, snapshot_data FROM "${schema}".kernel_snapshots WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT $2`,
      [tenantId, limit],
    );
    return result.rows.map((r: Record<string, any>) => ({
    ...
    logger.warn('[Kernel] listKernelSnapshots failed', { error: toErrorMessage(err) });
    return [];
  }
}
