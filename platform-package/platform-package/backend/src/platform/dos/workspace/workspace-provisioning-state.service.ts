...
function mapStepRow(r: any): ProvisioningStepRecord {
  return {
    stepId: r.step_id,
    runId: r.run_id,
...
export async function startProvisioning(
  ...
  if (steps.length === 0) {
    const err: unknown = new Error('Cannot start provisioning with zero steps');
    err.statusCode = 422;
    throw err;
  }
...
export async function retryStep(
  ...
  try {
    ...
    if (checkRows[0].status !== 'failed') {
      const err: unknown = new Error(`Step ${stepCode} is not in 'failed' state, cannot retry`);
      err.statusCode = 422;
      throw err;
    }
