  ...
  try {
    const res = await safeQuery(
      `SELECT role_code, role_name_en, role_name_ar, role_category,
              ...
              shahin_title_en AS product_title_en, shahin_title_ar AS product_title_ar, description_en, sort_order
    ...
    return res.rows.map((r: any) => ({
      ...
      recommendedFte: parseFloat(r.recommended_fte) || 1,
      isMandatory: r.is_mandatory,
      priority: r.priority || 50,
      productTitleEn: r.product_title_en ?? null,
      productTitleAr: r.product_title_ar ?? null,
      descriptionEn: r.description_en ?? null,
      sortOrder: r.sort_order || 0,
    }));
  } catch (err) {
