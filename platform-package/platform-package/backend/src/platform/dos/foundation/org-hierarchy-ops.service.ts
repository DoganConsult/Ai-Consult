...
export async function bulkCreateNodes(
  ...
  for (const node of nodes) {
    try {
      ...
      const vals: any[] = [id, node.name || '', 'active', userId];
...
export async function searchOrgStructure(
  ...
  const __pk = PK_MAP[nodeType] || 'org_id';
...
export async function exportOrgStructure(
  ...
  if (options.format === 'csv') {
    ...
    const lines = allNodes.map(n => `${n.type},${n.id},${(n.name || '').replace(/,/g, ';')},${n.status || ''},${n.head_user_id || ''},${n.created_at || ''}`);
    return {
...
export async function importOrgStructure(
  ...
  const result = await bulkCreateNodes(tenantId, userId, nodes.map(n => ({
    type: (n.type || 'organization') as OrgNodeType,
    name: n.name || n.org_name || n.bu_name || n.dept_name || n.team_name || '',
