...
import { ADMIN_ROLES } from '../../../modules/admin/security/admin.security';
...
/** @deprecated Fallback — runtime loads from org_node_type_registry table. */
const TABLE_MAP_FALLBACK: Record<string, string> = {
  organization: 'organizations', division: 'business_units',
  business_unit: 'business_units', department: 'departments',
  section: 'departments', team: 'teams', unit: 'teams',
  position: 'positions',
};
const PK_MAP_FALLBACK: Record<string, string> = {
  organization: 'org_id', division: 'bu_id', business_unit: 'bu_id',
  department: 'dept_id', section: 'dept_id', team: 'team_id',
  unit: 'team_id', position: 'position_id',
};
const NAME_COL_FALLBACK: Record<string, string> = {
  organization: 'org_name', division: 'bu_name', business_unit: 'bu_name',
  department: 'dept_name', section: 'dept_name', team: 'team_name',
  unit: 'team_name', position: 'title',
};

interface OrgNodeTypeMap {
  table: Record<string, string>;
  pk: Record<string, string>;
  name: Record<string, string>;
  ts: number;
}
const orgNodeTypeCache = new Map<string, OrgNodeTypeMap>();
const ORG_CACHE_TTL = 120_000;

async function __getOrgNodeTypeMaps(tenantId: string): Promise<{ table: Record<string, string>; pk: Record<string, string>; name: Record<string, string> }> {
  const cached = orgNodeTypeCache.get(tenantId);
  if (cached && Date.now() - cached.ts < ORG_CACHE_TTL) return cached;
  try {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(
      `SELECT node_type, table_name, pk_column, name_column FROM "${schema}".org_node_type_registry`,
    );
    const table: Record<string, string> = {};
    const pk: Record<string, string> = {};
    const name: Record<string, string> = {};
    for (const r of rows) {
      table[r.node_type] = r.table_name;
      pk[r.node_type] = r.pk_column;
      name[r.node_type] = r.name_column;
    }
    const entry = { table, pk, name, ts: Date.now() };
    orgNodeTypeCache.set(tenantId, entry);
    return entry;
  } catch {
    return { table: TABLE_MAP_FALLBACK, pk: PK_MAP_FALLBACK, name: NAME_COL_FALLBACK };
  }
}

// Sync fallback references for backward compatibility
const TABLE_MAP = TABLE_MAP_FALLBACK as Record<OrgNodeType, string>;
const PK_MAP = PK_MAP_FALLBACK as Record<OrgNodeType, string>;
const NAME_COL = NAME_COL_FALLBACK as Record<OrgNodeType, string>;

/** @deprecated Use getAdminProfiles() for DB-driven checks. Retained for sync contexts. */
const __ADMIN_ROLES_FALLBACK = new Set(['tenant_admin', 'platform_super_admin', 'owner', 'admin']);
...
function buildTree(rows: unknown[]): OrgHierarchyNode[] {
  ...
  for (const r of rows) {
    const node: OrgHierarchyNode = {
      type: r.node_type || 'organization',
      id: r.node_id,
      name: r.name_en || '',
      nameAr: r.name_ar ?? undefined,
      parentId: r.parent_node_id ?? undefined,
...
export async function getUserAccessibleDepartments(
  ...
  if (ADMIN_ROLES.has(userRole)) {
    ...
    return rows.map((r: unknown) => r.dept_id);
  }
...
export async function getUserAccessibleTeams(
  ...
  if (ADMIN_ROLES.has(userRole)) {
    ...
    return rows.map((r: unknown) => r.team_id);
  }
...
export async function upsertOrgHierarchyNode(
  ...
  nodeType: OrgNodeType,
  data: unknown,
): Promise<{ id: string; node: unknown }> {
  const schema = tenantSchema(tenantId);
  const table = TABLE_MAP[nodeType];
  const pk = PK_MAP[nodeType];
  ...
  if (isUpdate) {
    ...
    if (data.name !== undefined) { setClauses.push(`${nameCol} = $${idx}`); params.push(data.name); idx++; }
    if (data.nameAr !== undefined) { setClauses.push(`${nameCol.replace('_name', '_name_ar')} = $${idx}`); params.push(data.nameAr); idx++; }
    if (data.status !== undefined) { setClauses.push(`status = $${idx}`); params.push(data.status); idx++; }
    if (data.headUserId !== undefined) { setClauses.push(`head_user_id = $${idx}`); params.push(data.headUserId); idx++; }

    setClauses.push(`updated_at = NOW()`);
    setClauses.push(`updated_by = $${idx}`); params.push(userId); idx++;
    params.push(data.id); idx++;
    ...
    const { rows } = await safeQuery(`SELECT * FROM "${schema}".${table} WHERE ${pk} = $1`, [data.id]);
    return { id: data.id, node: rows[0] || data };
  }
  ...
  const vals = [id, data.name || '', data.status || 'active', userId];
  let paramIdx = 5;

  if (parentCol && data.parentId) {
    cols.push(parentCol);
