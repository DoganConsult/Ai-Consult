 ...
 *   CLICKHOUSE_DATABASE=platform_analytics
...
export function getClickHouseClient(): unknown {
  ...
  if (!_client) {
    try {
      ...
      if (!chHost || !chPort || !chDb) {
        logger.warn('[ClickHouse] CLICKHOUSE_HOST, CLICKHOUSE_PORT, or CLICKHOUSE_DATABASE not set.');
      }
      ...
      logger.error('[ClickHouse] Failed to create client:', err instanceof Error ? err.message : String(err));
      _enabled = false;
      return null;
    }
  }
...
export async function chQuery<T = Record<string, unknown>>(
  ...
  return resultSet.json() as Promise<T[]>;
}
...
export async function checkClickHouseHealth(): Promise<{
  ...
  try {
    const rows = await chQuery<{ version: string }>('SELECT version() AS version');
    const version = rows[0]?.version || 'unknown';
    return {
      healthy: true,
      version,
      database: process.env.CLICKHOUSE_DATABASE || 'platform_analytics',
    };
  } catch (err: unknown) {
    return {
      healthy: false,
...
export async function ensureClickHouseTables(): Promise<void> {
  ...
  for (const sql of ddl) {
    try {
      ...
      logger.warn('[ClickHouse] DDL failed:', err instanceof Error ? err.message : String(err));
    }
  }
