...
export function getPlatformConnectionConfig(): PlatformDbConfig {
  ...
  // B-04 FIX: Enforce SSL verification in production on non-local hosts.
  // PG_SSL=true must mean verified TLS, not "encrypted maybe".
  let sslRejectUnauthorized: boolean;
  if (isLocal) {
    // Local connections: SSL optional, no cert verification required
    sslRejectUnauthorized = false;
  } else if (isProduction) {
    // Production + remote host: require cert verification
    if (sslExplicit && !sslCaPath) {
      throw new Error(
        '[platform-db.config] FATAL: DB_SSL=true on remote host in production but DB_SSL_CA is not set. ' +
        'Cannot verify database server identity without a CA certificate. ' +
        'Set DB_SSL_CA to the path of your PostgreSQL server CA certificate, or set DB_SSL=false for unencrypted (NOT recommended).'
      );
    }
