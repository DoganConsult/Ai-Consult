...
DROP VIEW IF EXISTS v_user_effective_permissions CASCADE; CREATE VIEW v_user_effective_permissions AS
SELECT
  eup.user_id,
  eup.permission_code,
  eup.module_code,
...
DROP VIEW IF EXISTS v_user_visible_modules CASCADE; CREATE VIEW v_user_visible_modules AS
SELECT
  eum.user_id,
  eum.module_code,
  eum.access_level,
...
DROP VIEW IF EXISTS v_module_catalog CASCADE; CREATE VIEW v_module_catalog AS
SELECT
  pm.code,
  pm.name_en,
  pm.name_ar,
...
DROP VIEW IF EXISTS v_role_bundle_permissions CASCADE; CREATE VIEW v_role_bundle_permissions AS
SELECT
...
DROP VIEW IF EXISTS v_archetype_policy_summary CASCADE; CREATE VIEW v_archetype_policy_summary AS
SELECT
  tb.archetype_code,
  map.module_code,
  map.activation_status AS module_activation,
  mwp.approval_style,
  mwp.sod_strictness,
  mwp.sla_multiplier,
...
DROP VIEW IF EXISTS v_workflow_state_machine CASCADE; CREATE VIEW v_workflow_state_machine AS
SELECT
  wps.archetype_code,
  wps.module_code,
  wps.state_code,
...
--    Guarded: only created if ai_autonomy_state table exists.
DROP VIEW IF EXISTS v_ai_autonomy_resolved CASCADE;
DO $$
BEGIN
  IF EXISTS (
    ...
    RAISE NOTICE 'v_ai_autonomy_resolved view created';
  ELSE
    RAISE NOTICE 'Skipping v_ai_autonomy_resolved: ai_autonomy_state not yet created';
  END IF;
END $$;
...
DROP VIEW IF EXISTS v_approver_chain CASCADE; CREATE VIEW v_approver_chain AS
SELECT
  arc.module_code,
  arc.transition_key,
...
DROP VIEW IF EXISTS v_active_overrides CASCADE; CREATE VIEW v_active_overrides AS
SELECT
  ro.id,
  ro.override_type,
  ro.target_key,
