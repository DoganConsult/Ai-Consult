...
export function createOpenClawServer(): OpenClawServer {
  ...
  // CORS
  if (process.env.NODE_ENV === 'production' && config.corsOrigins.includes('*')) {
    throw new Error(
      'OpenClaw CORS_ORIGINS contains wildcard "*" in production. '
      + 'This is forbidden — set explicit allowed origins.'
    );
  }
