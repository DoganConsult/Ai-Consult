 ...
 *   product-agrc     — Shahin GRC product domains (risk, compliance, audit, governance, etc.)
...
import { getAllRegisteredModuleCodes } from '../dos/modules/registry/platform-module-registry';
import {
...
const CANONICAL_OWNERSHIP: Record<CanonicalModuleCode, ModuleOwnership> = {
  ...
  // ── Product AGRC (Shahin GRC business domains) ── hierarchyLayer: 'product'
  ...
  'quality-gate': { code: 'quality-gate', tier: 'platform-core', hierarchyLayer: 'platform', canonicalServiceDir: 'modules/quality-gate/services', mountPrefix: '/api/quality-gate', permissionFamily: 'quality-gate', eventNamespace: 'quality-gate', labelEn: 'Quality Gates' },
};

// ── Non-Canonical Directory Ownership ───────────────────────────────────
// Directories that exist in modules/ but don't map to a canonical code.
// These are either platform infrastructure, product extensions, or shared internals.
...
export const DIRECTORY_OWNERSHIP: DirectoryOwnership[] = [
...
export function getModuleOwnership(code: CanonicalModuleCode): ModuleOwnership {
  return CANONICAL_OWNERSHIP[code];
}

/** Get all modules belonging to a given tier. */
export function getModulesByTier(tier: OwnershipTier): ModuleOwnership[] {
...
export function getDirectoryTier(directory: string): OwnershipTier | undefined {
  const entry = DIRECTORY_OWNERSHIP.find(d => directory.startsWith(d.directory));
  return entry?.tier;
}

/** Get all canonical module ownership entries. */
export function getAllModuleOwnership(): Record<CanonicalModuleCode, ModuleOwnership> {
  return { ...CANONICAL_OWNERSHIP };
}

/** Validate that all 25 canonical codes are covered. Throws if any are missing. */
export function validateOwnershipCompleteness(): string[] {
  const errors: string[] = [];
...
export function getModulesByHierarchyLayer(layer: HierarchyLayer): ModuleOwnership[] {
...
export function validateHierarchyLayerConsistency(): string[] {
  const errors: string[] = [];
  const tierToLayer: Record<OwnershipTier, HierarchyLayer> = {
