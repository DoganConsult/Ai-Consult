  ...
  await safeQuery(
    `INSERT INTO "${ctx.schema}".tenant_settings
     ...
     ON CONFLICT (key) DO UPDATE SET
       ...
       owner_user_id = $7,
       updated_at = now()`,
    [key, serialized, ctx.scope, ctx.productKey ?? null, ctx.moduleCode ?? null, ctx.workspaceId ?? null, ctx.ownerUserId ?? null]
  );
}
...
export async function resolveSettingWithInheritance(
  ...
  if (context.ownerUserId) {
    scopes.push({ schema, scope: 'user', ownerUserId: context.ownerUserId, workspaceId: context.workspaceId, moduleCode: context.moduleCode, productKey: context.productKey });
  }
...
export async function detectScopeAmbiguities(schema: string): Promise<string[]> {
  ...
  try {
    const result = await safeQuery(
      `SELECT key, scope, module_code, product_key, workspace_id, owner_user_id
    ...
    for (const row of result.rows) {
      ...
      if (row.module_code && row.scope !== 'module') {
        errors.push(`Setting '${row.key}' has module_code='${row.module_code}' but scope='${row.scope}' (expected 'module')`);
      }
    }
  } catch (err: unknown) {
    logger.warn(`[SettingsResolver] Could not read tenant_settings from ${schema}: ${toErrorMessage(err)}`);
  }
...
async function resolveSettingRow(
  ...
  return getFirstRow(result) ?? undefined;
}

function mapRow(row: GenericRow): SettingRecord {
  return {
    key: row.key,
