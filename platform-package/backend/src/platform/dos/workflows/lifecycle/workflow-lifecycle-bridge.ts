...
import { canTransition, performTransition, type TransitionResult } from '../../lifecycle/lifecycle-engine';
import { isTransitionValid, getTransitionPermission } from '../../lifecycle/lifecycle-registry';
...
async function checkSod(tenantId: string, userId: string, moduleCode: string, entityType: string, entityId: string): Promise<{ passed: boolean; detail: string }> {
  try {
    ...
    const recentActions = rows.map((r: any) => r.action_type).filter(Boolean);
...
export async function executeLifecycleTransition(request: TransitionRequest): Promise<TransitionDecision> {
  ...
  if (requiresApprovalWorkflow) {
    ...
    }, { userId, moduleCode, entityType, entityId, category: 'lifecycle' }).catch(() => {});
...
export async function approveTransition(
  ...
  if (decision === 'rejected') {
    ...
    }, { userId: approverId, moduleCode: task.module_code, entityType: task.entity_type, entityId: task.entity_id, category: 'lifecycle' }).catch(() => {});
