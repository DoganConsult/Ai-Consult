...
import type { ApprovalPreScreen } from '../../../../types/cooperative-workflows.types';
...
export async function preScreenApproval(tenantId: string, approvalId: string): Promise<ApprovalPreScreen> {
  ...
  const approvalRes = await safeQuery(
    `SELECT * FROM "${schema}".approval_requests WHERE approval_id = $1`, [approvalId],
  );
  if (!approvalRes.rows.length) throw new Error('Approval request not found');
  const approval = getFirstRow(approvalRes);

  // Analyze based on entity type
  const analysis = await analyzeEntity(schema, approval.entity_type, approval.entity_id);

  const recommendation = analysis.gapsFound.length === 0 && analysis.confidenceScore >= 70
    ? 'approve' : analysis.gapsFound.length > 3 ? 'reject' : 'needs_review';

  const res = await safeQuery(
    `INSERT INTO "${schema}".approval_pre_screens
    ...
    [approvalId, recommendation, JSON.stringify(analysis.supportingEvidence),
     JSON.stringify(analysis.gapsFound), analysis.confidenceScore, analysis.summary],
  );

  const preScreen: ApprovalPreScreen = {
    preScreenId: getFirstRow(res)?.pre_screen_id,
    approvalId, agentId: 'AGENT-A01', recommendation,
  ...
  await eventBus.publish({
    tenantId, eventType: 'approval.pre_screened', severity: 'info',
    entityId: approvalId,
    payload: { recommendation, confidenceScore: analysis.confidenceScore, gapCount: analysis.gapsFound.length },
  });
...
function mapPreScreen(r: any): ApprovalPreScreen {
  return {
