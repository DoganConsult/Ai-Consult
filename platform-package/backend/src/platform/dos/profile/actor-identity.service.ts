...
import type {
  ...
  DecisionOutcome,
  AuthorityType,
} from '../../../types/actor-identity.types';
...
export async function checkDecisionAuthority(
  ...
  const actorRoles = roleRes.rows.map((r: any) => r.role_code as string);

  const hasRequiredRole = authority.requiredRoleCodes.length === 0 ||
    authority.requiredRoleCodes.some((rc: any) => actorRoles.includes(rc));

  if (!hasRequiredRole) {
    return {
      authorized: false,
      authority,
      reason: `Actor lacks required role. Needs one of: ${authority.requiredRoleCodes.join(', ')}`,
    };
  }

  // TODO(dauth-migration): SoD check belongs in platform/dauth/sod/sod-policy.service — migrate in Phase 3
  if (authority.requiresSodSeparation && authority.sodConflictRoles.length > 0) {
    const hasConflict = authority.sodConflictRoles.some((rc: any) => actorRoles.includes(rc));
    if (hasConflict) {
      return {
        authorized: false,
        authority,
        reason: `SoD conflict: actor holds conflicting role from ${authority.sodConflictRoles.join(', ')}`,
      };
    }
  }
...
function mapActorRow(row: any): ActorIdentity {
  return {
...
function mapAccessProfileRow(row: any): AccessProfile {
  return {
...
function mapFunctionalRoleRow(row: any): FunctionalRole {
  return {
    roleId: row.role_id,
...
function mapDecisionAuthorityRow(row: any): DecisionAuthority {
  return {
    id: row.id,
