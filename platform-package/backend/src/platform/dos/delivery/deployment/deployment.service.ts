...
export async function listDeployments(tenantId: string, environment?: string): Promise<DeploymentRecord[]> {
  ...
  const result = environment
    ? await safeQuery(
        `SELECT * FROM "${schema}".dos_deployments WHERE environment = $1 ORDER BY started_at DESC`,
        [environment],
      )
    : await safeQuery(`SELECT * FROM "${schema}".dos_deployments ORDER BY started_at DESC`, []);
...
function mapDeploymentRow(row: Record<string, unknown>): DeploymentRecord {
  return {
