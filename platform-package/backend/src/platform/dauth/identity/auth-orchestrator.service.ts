...
import { withTimeout } from '../../dos/operations/reliability/reliability.service';
import {
  createEmailChallenge,
  _verifyEmailChallenge,
  type _MfaType,
} from '../mfa/mfa.service';
import { SYSTEM_TENANT } from '../../dos/constants/system-actors';
...
interface AuthStoredUser extends AuthenticatedUser {
...
const LOGIN_CONTEXT_TIMEOUT_MS = 1200;
const OPTIONAL_AUTHZ_TIMEOUT_MS = 700;
...
function buildUserFromRow(row: Record<string, any>): AuthenticatedUser {
  return {
    user_id: row.user_id,
    email: row.email,
...
export async function issueLoginTokens(
  ...
  if (dec?.jti) {
    await registerActiveJtiForUser(user.user_id, dec.jti, getAccessTokenExpirySeconds()).catch(() => {});
  }
...
export async function completeMfaLogin(
  ...
  if (user.status === 'suspended' || user.status === 'inactive' || user.status === 'deactivated') {
    return { user, tokens: null as any, allRoles: [], effectiveRole: '', orgName: '', tenantMemberships: [], enterpriseAuthz: null, sessionId: null };
  }
...
export async function changePassword(
  ...
  const userRes = await query(`SELECT password_hash, email, role FROM users WHERE user_id = $1`, [userId]);
  ...
  await query(`UPDATE users SET password_hash = $1, must_change_password = FALSE WHERE user_id = $2`, [hash, userId]);
...
export async function resolveOrgName(tenantId: string): Promise<string> {
  try {
    ...
    return tenantResult.rows[0]?.org_name || '';
  } catch {
    return '';
  }
}
...
export async function resolveLoginBootstrapData(
  ...
  const [orgName, tenantMemberships, sessionId, enterpriseAuthz] = await Promise.all([
    ...
    withTimeout(() => resolveOnboardingSessionId(userId, onboardingComplete), LOGIN_CONTEXT_TIMEOUT_MS).catch(() => null),
    withTimeout(() => resolveEnterpriseAuthz(tenantId, userId), OPTIONAL_AUTHZ_TIMEOUT_MS).catch(() => null),
  ]);
 ...
 */
export async function resolveEnterpriseAuthz(tenantId: string, userId: string): Promise<unknown> {
  try {
    // Lazy import to avoid circular dependency
    // Lazy import to avoid circular dependency
    const { accessSnapshotService } = await import('../access/access-snapshot.service');
