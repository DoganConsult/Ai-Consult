...
export function getJwtSecret(): string {
  ...
  const secret = process.env.DAUTH_JWT_SECRET || process.env.JWT_SECRET;
  if (secret) { _jwtSecret = secret; return secret; }
  if (process.env.NODE_ENV === 'production') {
    throw new Error('[DAuth] FATAL: DAUTH_JWT_SECRET environment variable is required in production');
  }
  if (process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'test') {
    logger.warn('[DAuth] DAUTH_JWT_SECRET not set — using dev fallback. DO NOT deploy to production.');
    _jwtSecret = 'dev-secret';
    return _jwtSecret;
  }
  throw new Error('DAUTH_JWT_SECRET environment variable is required');
}

function getJwtRefreshSecret(): string {
  if (_jwtRefreshSecret) return _jwtRefreshSecret;
  const secret = process.env.DAUTH_JWT_REFRESH_SECRET || process.env.JWT_REFRESH_SECRET;
  if (secret) { _jwtRefreshSecret = secret; return secret; }
  if (process.env.NODE_ENV === 'production') {
    throw new Error('[DAuth] FATAL: DAUTH_JWT_REFRESH_SECRET environment variable is required in production');
  }
  if (process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'test') {
    logger.warn('[DAuth] DAUTH_JWT_REFRESH_SECRET not set — using dev fallback. DO NOT deploy to production.');
    _jwtRefreshSecret = getJwtSecret() + '_refresh';
    return _jwtRefreshSecret;
  }
  throw new Error('DAUTH_JWT_REFRESH_SECRET environment variable is required');
}

// ── Access Tokens ──

export function generateAccessToken(payload: AuthPayload): string {
  const expiresIn = process.env.DAUTH_JWT_EXPIRES_IN || process.env.JWT_EXPIRES_IN || '15m';
