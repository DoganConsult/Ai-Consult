...
import {
  ...
  verifyRefreshToken,
  _setRefreshTokenCookie,
  _clearRefreshTokenCookie,
...
async function persistSession(params: {
  ...
  if (activeCount >= MAX_CONCURRENT_SESSIONS) {
    await safeQuery(
      `UPDATE public.sessions SET revoked_at = NOW() WHERE session_id = (
        SELECT session_id FROM public.sessions WHERE user_id = $1 AND revoked_at IS NULL ORDER BY created_at ASC LIMIT 1
      )`,
      [params.userId],
    );
  }
...
export async function refreshSession(
  ...
  if (decoded.jti) {
    await blacklistToken(decoded.jti, decoded.userId, 60).catch(() => {});
  ...
  logger.info('[DAuth] Token refreshed', { userId: decoded.userId, tenantId: decoded.tenantId, ip: (decoded as any).ip });
...
export async function destroySession(userId: string, jti?: string): Promise<void> {
  if (jti) {
    await blacklistToken(jti, userId).catch(() => {});
