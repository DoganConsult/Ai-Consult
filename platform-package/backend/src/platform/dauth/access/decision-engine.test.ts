...
function defaultQueryHandler(sql: string): { rows: any[] } {
...
describe('DAuth Decision Engine — 14-step pipeline', () => {
  ...
  it('logs decision to authz_decision_log', async () => {
    ...
    const logCalls = mockSafeQuery.mock.calls.filter(
      (c: any[]) => typeof c[0] === 'string' && c[0].includes('authz_decision_log'),
    );
  ...
  // ── Step 13: Delegation/ownership tests ──

  it('step 13: passes when no entity context', async () => {
    const d = await evaluateAccess(baseCtx());
    expect(d.steps[12].name).toBe('delegation_ownership_pass');
    expect(d.steps[12].passed).toBe(true);
    expect(d.steps[12].detail).toBe('No entity context');
  });

  it('step 13: passes when entity has no ownership record', async () => {
    mockSafeQuery.mockImplementation(async (sql: string) => {
      if (sql.includes('entity_ownership')) return { rows: [] };
      return defaultQueryHandler(sql);
    });
    const d = await evaluateAccess(baseCtx({ entityId: 'e-001', entityType: 'risk_record' }));
    expect(d.steps[12].passed).toBe(true);
    expect(d.steps[12].detail).toContain('owner');
  });

  it('step 13: passes when user owns the entity', async () => {
    mockSafeQuery.mockImplementation(async (sql: string) => {
      if (sql.includes('entity_ownership')) return { rows: [{ owner_user_id: 'u-001' }] };
      return defaultQueryHandler(sql);
    });
    const d = await evaluateAccess(baseCtx({ entityId: 'e-001', entityType: 'risk_record' }));
    expect(d.steps[12].passed).toBe(true);
  ...
  it('step 13: passes when entity is owned by another user but delegation exists', async () => {
    mockSafeQuery.mockImplementation(async (sql: string) => {
      if (sql.includes('entity_ownership')) return { rows: [{ owner_user_id: 'u-other' }] };
      if (sql.includes('delegations')) return { rows: [{ id: 1 }] };
      return defaultQueryHandler(sql);
    });
    const d = await evaluateAccess(baseCtx({ entityId: 'e-001', entityType: 'risk_record' }));
    expect(d.steps[12].passed).toBe(true);
    expect(d.steps[12].detail).toBe('Active delegation found');
  });

  it('step 13: DENIES when entity owned by another user and no delegation', async () => {
    mockSafeQuery.mockImplementation(async (sql: string) => {
      if (sql.includes('entity_ownership')) return { rows: [{ owner_user_id: 'u-other' }] };
      if (sql.includes('delegations')) return { rows: [] };
      return defaultQueryHandler(sql);
    ...
    const d = await evaluateAccess(baseCtx({ entityId: 'e-001', entityType: 'risk_record' }));
    expect(d.allowed).toBe(false);
    expect(d.failedStep).toBe(13);
    expect(d.failedCheck).toBe('delegation_ownership_pass');
  });

  it('step 13: super-admin bypasses ownership/delegation check', async () => {
    mockSafeQuery.mockImplementation(async (sql: string) => {
      if (sql.includes('entity_ownership')) return { rows: [{ owner_user_id: 'u-other' }] };
      if (sql.includes('delegations')) return { rows: [] };
      return defaultQueryHandler(sql);
    });
    const d = await evaluateAccess(baseCtx({ entityId: 'e-001', entityType: 'risk_record', isSuperAdmin: true }));
    expect(d.steps[12].passed).toBe(true);
    expect(d.steps[12].detail).toBe('Super-admin bypass');
  });

  it('step 13: gracefully handles missing entity_ownership table', async () => {
    mockSafeQuery.mockImplementation(async (sql: string) => {
      if (sql.includes('entity_ownership')) throw new Error('relation "entity_ownership" does not exist');
      return defaultQueryHandler(sql);
    });
    const d = await evaluateAccess(baseCtx({ entityId: 'e-001', entityType: 'risk_record' }));
    expect(d.steps[12].passed).toBe(true);
