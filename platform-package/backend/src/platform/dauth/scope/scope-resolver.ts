...
export async function resolveFullHierarchy(tenantId: string, userId: string): Promise<FullScopeHierarchy> {
  ...
  for (const orgId of orgIds) {
    ...
    const orgBuIds = orgBUs.rows.map((r: any) => r.bu_id);
    ...
    const orgDepts = orgBuIds.length > 0
      ...
      : { rows: [] };
    const orgDeptIds = orgDepts.rows.map((r: any) => r.dept_id);
    ...
    const orgSections = orgDeptIds.length > 0
      ...
      : { rows: [] };
    const orgSectionIds = orgSections.rows.map((r: any) => r.dept_id);
    ...
    const orgTeams = allOrgDeptIds.length > 0
      ...
      : { rows: [] };
    const orgTeamIds = orgTeams.rows.map((r: any) => r.team_id);
    ...
    const orgPositions = allOrgDeptIds.length > 0
      ...
      : { rows: [] };
    const orgPositionIds = orgPositions.rows.map((r: any) => r.position_id);
