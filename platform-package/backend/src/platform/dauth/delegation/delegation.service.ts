...
import { publish as publishEvent } from '../../dos/events/event-bus';
...
export async function createDelegationGrant(
  ...
  await swallow(EC.EVENT_BUS, publishEvent(
    ...
    { grantId, userId, agentId, scopes },
    { userId, moduleCode: 'delegation', category: 'security' },
...
export async function revokeDelegationGrant(
  ...
  try {
    ...
    if (jtis.rows.length > 0) {
      const { blacklistToken } = await import('../session/token-blacklist.service');
      for (const row of jtis.rows) {
        await blacklistToken(row.jti).catch(() => {});
      }
    }
  } catch {
...
export async function executeDelegatedAction(
  ...
  userId: string,
  agentId: string,
  action: { type: string; title: string; description: string; priority: string; entityType?: string; entityId?: string; assignToRole?: string; dueInDays?: number; [key: string]: unknown },
  ...
  // 3. Execute via agent-runner (optional — product module)
  let executeAction: any;
  try { ({ executeAction } = await import('../../../modules/ai/services/agents/core/agent-runner.service')); } catch {
    throw new Error('Agent runner not available in standalone platform package');
  }
  await executeAction(tenantId, agentId, action as any);
  ...
  await swallow(EC.EVENT_BUS, publishEvent(
    ...
    { grantId: grant.grantId, agentId, userId, actionType: action.type },
    { userId, moduleCode: 'delegation', category: 'security', severity: action.priority === 'critical' ? 'warn' : 'info' },
...
export async function recordDelegatedAction(
  ...
  payload: Record<string, any>, result: 'success' | 'failure', errorMessage: string | null = null,
...
export async function requestDelegation(tenantId: string, data: {
  ...
  try {
    const { submitForApproval } = await import('../../dos/workflows/approvals/approval-engine.service');
  ...
  await swallow(EC.EVENT_BUS, publishEvent(
    ...
    { authority_type: data.authority_type, delegate: data.delegate_user_id, requiresApproval },
    { userId: data.requested_by, moduleCode: 'delegation', category: 'security' },
...
export async function rejectDelegation(tenantId: string, delegationId: string, _rejectedBy: string, reason: string): Promise<GenericRow | undefined> {
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `UPDATE "${schema}".governance_delegations
...
export async function updateDelegation(tenantId: string, delegationId: string, data: Record<string, any>): Promise<GenericRow | undefined> {
  const schema = tenantSchema(tenantId);
...
export async function revokeDelegation(tenantId: string, delegationId: string): Promise<GenericRow | undefined> {
  ...
  await swallow(EC.EVENT_BUS, publishEvent(
    ...
    { authority_type: revoked.authority_type, delegate_user_id: revoked.delegate_user_id },
    { userId: revoked.delegator_user_id, moduleCode: 'delegation', category: 'security', severity: 'warn' },
...
export async function getExpiringDelegations(tenantId: string, withinDays: number = 7): Promise<GenericRow[]> {
  ...
  for (const d of result.rows) {
    await swallow(EC.EVENT_BUS, publishEvent(
      ...
      { authority_type: d.authority_type, delegate_user_id: d.delegate_user_id, valid_to: d.valid_to },
      { userId: d.delegator_user_id || SYSTEM_JOB_ACTOR, moduleCode: 'delegation', category: 'domain', severity: 'warn' },
  ...
  if (count > 0) {
    for (const row of result.rows) {
      await swallow(EC.EVENT_BUS, publishEvent(
        ...
        { userId: SYSTEM_JOB_ACTOR, moduleCode: 'delegation', category: 'domain', severity: 'warn' },
      ), { tenantId, operation: 'eventBus:governance.delegation_expired' });
    }
  }
  return count;
}
export function ACTION_TYPE_TO_SCOPE(..._args: any[]): unknown { return undefined; }
