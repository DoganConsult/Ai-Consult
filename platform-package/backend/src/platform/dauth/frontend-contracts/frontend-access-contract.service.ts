 ...
 */
function computeContractHash(data: Record<string, unknown>): string {
...
function snapshotToContract(snapshot: AccessSnapshot, versionHash: string): FrontendAccessContract {
  return {
    ...
    decisionAuthorities: [...(snapshot as Record<string, unknown>).decisionAuthorities ?? []],
    accessProfiles: [...(snapshot as Record<string, unknown>).accessProfiles ?? []],
  };
}
