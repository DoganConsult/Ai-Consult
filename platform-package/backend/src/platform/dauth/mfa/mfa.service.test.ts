...
// ── Additional tests for TOTP enrollment, lockout, and expired challenge ──

describe('DAuth MFA — TOTP enrollment flow', () => {
  it('enableMfa with totp type stores secret', async () => {
    mockQuery.mockResolvedValue({ rows: [], rowCount: 1 });
    await enableMfa('u-totp', 'totp', 'BASE32SECRET');
    const [sql, params] = mockQuery.mock.calls[0];
    expect(sql).toContain('INSERT INTO user_mfa');
    expect(params[1]).toBe('totp');
    expect(params[2]).toBe('BASE32SECRET');
  });

  it('enableMfa with email type passes null secret', async () => {
    mockQuery.mockResolvedValue({ rows: [], rowCount: 1 });
    await enableMfa('u-email', 'email');
    const [, params] = mockQuery.mock.calls[0];
    expect(params[1]).toBe('email');
...
describe('DAuth MFA — challenge/response patterns', () => {
  it('verifyEmailChallenge verifies code against token table with expiry check', async () => {
    mockQuery.mockResolvedValueOnce({ rows: [{ '?column?': 1 }] }); // found
    mockQuery.mockResolvedValueOnce({ rows: [], rowCount: 1 }); // delete
    const result = await verifyEmailChallenge('u-001', '654321');
    expect(result).toBe(true);
  });

  it('verifyEmailChallenge returns false for expired token (no match)', async () => {
    mockQuery.mockResolvedValueOnce({ rows: [] }); // not found / expired
    const result = await verifyEmailChallenge('u-001', '999999');
    expect(result).toBe(false);
  });
});

describe('DAuth MFA — lockout after failed attempts', () => {
  it('repeated failed verifications all return false', async () => {
    // Simulate 5 failed attempts — each returns no match
    for (let i = 0; i < 5; i++) {
      mockQuery.mockResolvedValueOnce({ rows: [] });
    }
    for (let i = 0; i < 5; i++) {
      const result = await verifyEmailChallenge('u-locked', `00000${i}`);
      expect(result).toBe(false);
    }
...
describe('DAuth MFA — expired challenge handling', () => {
  it('createEmailChallenge sets expiry ~10 minutes in the future', async () => {
    mockQuery.mockResolvedValue({ rows: [], rowCount: 1 });
    const { expiresAt } = await createEmailChallenge('u-001', 't-1');
    const diffMs = expiresAt.getTime() - Date.now();
    expect(diffMs).toBeGreaterThan(9 * 60_000);
    expect(diffMs).toBeLessThanOrEqual(10 * 60_000 + 500); // small tolerance
  });

  it('expired token is not matched by verifyEmailChallenge', async () => {
    // The SELECT query checks `expires_at > NOW()` — expired tokens won't match
    mockQuery.mockResolvedValueOnce({ rows: [] });
    const result = await verifyEmailChallenge('u-001', '123456');
    expect(result).toBe(false);
