import { initMasterDB, migrateAllTenants } from '../config/database/database';
...
export async function runMigrationsAndSeedsPhase(): Promise<void> {
  ...
  try {
    ...
    logger.error('Master migrations failed', { error: toErrorMessage(e) });
    // Production: migrations must succeed — crash fast rather than serve on stale schema
  ...
  await seedPlatformAdmin().catch((e: unknown) => logger.warn('Platform admin seed skipped', { error: toErrorMessage(e) }));
}
