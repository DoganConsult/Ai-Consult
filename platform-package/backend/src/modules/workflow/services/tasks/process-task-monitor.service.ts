...
/**
 * Distributes a task to an available member of the specified team.
 * Selects the team member with the fewest active tasks (round-robin by load).
 * Falls back to team lead if no members found.
 */
async function distributeTask(tenantId: string, teamId: string, _context: string): Promise<string | null> {
  if (!teamId) return null;
  const schema = tenantSchema(tenantId);

  // Find team member with lowest active task count
  const result = await safeQuery(
    `SELECT tm.user_id
     FROM "${schema}".team_members tm
     LEFT JOIN "${schema}".process_tasks pt
       ON pt.assigned_user_id = tm.user_id
       AND pt.status NOT IN ('completed', 'cancelled', 'auto_closed')
     WHERE tm.team_id = $1
       AND tm.is_active = true
     GROUP BY tm.user_id
     ORDER BY COUNT(pt.task_id) ASC
     LIMIT 1`,
    [teamId],
  );
  ...
  // Fallback: try team lead
  const leadResult = await safeQuery(
    `SELECT lead_user_id FROM "${schema}".teams WHERE id = $1 AND lead_user_id IS NOT NULL`,
    [teamId],
  );
  return leadResult.rows.length > 0 ? leadResult.rows[0].lead_user_id : null;
}
...
import { swallowNull, swallowDefault, EC , catchHandler , swallow } from '../../../../utils/resilient-catch';
...
export async function checkProcessTaskSLAs(tenantId: string): Promise<SLACheckResult> {
  ...
  for (const task of breachedRes.rows) {
    if (task.escalation_level <= 3 && task.team_id) {
      ...
      }), { tenantId, operation: 'eventBus:process_task.sla_breached' });
  ...
  for (const task of warningRes.rows) {
    ...
    }), { tenantId, operation: 'eventBus:process_task.sla_warning' });
  }
...
export async function detectStaleTasks(tenantId: string): Promise<StaleTaskDetectionResult> {
  ...
  for (const task of stale7DaysRes.rows) {
    ...
    }), { tenantId, operation: 'eventBus:process_task.stale_detected' });
  }
  ...
  for (const task of stale14DaysRes.rows) {
    ...
    if (task.team_id) {
      try {
        ...
        if (escPath.rows.length > 0) {
          const escTeamId = getFirstRow(escPath)?.escalate_to_team_id;
          const newAssignee = await swallowNull(EC.FALLBACK_QUERY, distributeTask(tenantId, escTeamId, 'escalation'), { tenantId: tenantId, operation: 'query team_escalation_paths' });
          if (newAssignee) {
            await safeQuery(
              `UPDATE "${schema}".process_tasks
               SET team_id = $1, assigned_user_id = $2, status = 'escalated',
    ...
    }), { tenantId, operation: 'eventBus:process_task.stale_critical' });
  }
