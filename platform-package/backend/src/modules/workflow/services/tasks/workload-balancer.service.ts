...
import { getAgentResourceAllocation } from '../../../../platform/dos/core/infrastructure/resource-allocator.service';
...
export async function getAgentWorkloadMetrics(
  ...
  const allocation = await getAgentResourceAllocation(tenantId, agentId);
  const baseCapacity = allocation?.computeUnits ? Math.floor(allocation.computeUnits / 10) : 5;
