...
import { asyncHandler } from '../../../openclaw/middleware/error-handler';
import { NotFoundError } from "../../../errors";
import { createPushtokenBody, updatePreferencesBody, updateIdReadBody, updateReadallBody } from '../schemas/notification.schemas';
import { updateReadallBody, updatePreferencesBody, createPushtokenBody, updateIdReadBody } from '../schemas/notification.schemas';
import {
  getNotifications,
  markAsRead,
  markAllAsRead,
...
import { requireOwnership } from '../../../platform/dos/http/guards/require-ownership';
import { swallow, swallowDefault, EC , catchHandler } from '../../../platform/dos/resilience/resilient-catch';
...
router.get("/", authenticate, requirePermission("notification.config.read"), asyncHandler(async (req: Request, res: Response) => {
...
router.put("/:id/read", authenticate, requirePermission("notification.config.write"), requireOwnership('notification'), validate({ body: updateIdReadBody }), asyncHandler(async (req: Request, res: Response) => {
  const tenantId = req.user.tenantId as string;
  await markAsRead(tenantId, req.params.id);
  setAuditData(res, { action: "update", entityType: "notification", entityId: req.params.id, afterState: { read: true } });
  swallow(EC.EVENT_BUS, emitEvent({ tenantId, userId: req.user!.userId, module: 'admin', event: 'updated', entityType: 'notification', entityId: req.params.id }), { tenantId: tenantId, operation: 'grcEvent:admin.notification.updated' });
...
router.put("/read-all", authenticate, requirePermission("notification.config.write"), validate({ body: updateReadallBody }), asyncHandler(async (req: Request, res: Response) => {
  ...
  setAuditData(res, { action: "update", entityType: "notification", entityId: userId, afterState: { allRead: true } });
  swallow(EC.EVENT_BUS, emitEvent({ tenantId, userId, module: 'admin', event: 'updated', entityType: 'notification', entityId: userId }), { tenantId: tenantId, operation: 'grcEvent:admin.notification.updated' });
  res.ok({ message: "All notifications marked as read" });
}));

// DELETE /api/notifications/:id — Delete a notification
router.delete("/:id", authenticate, requirePermission("notification.config.write"), requireOwnership('notification'), asyncHandler(async (req: Request, res: Response) => {
  const tenantId = req.user.tenantId;
  const deleted = await deleteNotification(tenantId, req.params.id) as string;
  if (!deleted) throw new NotFoundError("Notification", req.params.id);
  setAuditData(res, { action: "delete", entityType: "notification", entityId: req.params.id, afterState: { deleted: true } });
  swallow(EC.EVENT_BUS, emitEvent({ tenantId, userId: req.user!.userId, module: 'admin', event: 'deleted', entityType: 'notification', entityId: req.params.id }), { tenantId: tenantId, operation: 'grcEvent:admin.notification.deleted' });
...
router.get("/preferences", authenticate, requirePermission("notification.config.read"), asyncHandler(async (req: Request, res: Response) => {
  const tenantId = req.user.tenantId;
  const userId = req.user.userId;
...
router.put("/preferences", authenticate, requirePermission("notification.config.write"), validate({ body: updatePreferencesBody }), asyncHandler(async (req: Request, res: Response) => {
  ...
  swallow(EC.EVENT_BUS, emitEvent({ tenantId, userId, module: 'admin', event: 'updated', entityType: 'notification_preferences', entityId: userId }), { tenantId: tenantId, operation: 'grcEvent:admin.notification_preferences.updated' });
  res.ok({ preferences: prefs });
}));
...
router.delete("/push-token", authenticate, asyncHandler(async (req: Request, res: Response) => {
  const tenantId = req.user.tenantId;
  const userId = req.user.userId;
...
router.get("/smart-digest", authenticate, requirePermission("notification.config.read"), asyncHandler(async (req: Request, res: Response) => {
  const tenantId = req.user.tenantId;
  const userId = req.user.userId;
  const { safeQuery, tenantSchema } = await import("../../../config/database/database");
  const schema = tenantSchema(tenantId);
  ...
  const { claudeJSON } = await import('../../../config/app/claude-client');
...
router.post("/ai-prioritize", authenticate, requirePermission("notification.config.write"), asyncHandler(async (req: Request, res: Response) => {
  const tenantId = req.user.tenantId;
  const userId = req.user.userId;
  const { safeQuery, tenantSchema } = await import("../../../config/database/database");
  const schema = tenantSchema(tenantId);
...
router.get("/analytics", authenticate, requirePermission("notification.config.read"), asyncHandler(async (req: Request, res: Response) => {
  const tenantId = req.user.tenantId;
  const { safeQuery, tenantSchema } = await import("../../../config/database/database");
  const schema = tenantSchema(tenantId);
