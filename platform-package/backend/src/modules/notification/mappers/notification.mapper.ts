// @ts-nocheck
import type { Notification, NotificationCreateInput } from '../types/notification.types';
import type { GenericRow } from '../../../types/db-rows.types';

export function toEntity(row: GenericRow): Notification {
  return {
...
export function toEntityList(rows: GenericRow[]): Notification[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): NotificationCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
...
export function toApiResponse(entity: Notification): Record<string, unknown> {
  const { deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: Notification[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
...
export function toListItem(entity: Record<string, unknown>): Record<string, unknown> {
  const { deleted_at, internal_notes, system_flags, ai_metadata, ...rest } = entity;
  return rest;
}
