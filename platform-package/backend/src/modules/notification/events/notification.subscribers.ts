...
import { eventBus, type PlatformEvent } from '../../../platform/dos/events/event-bus';
...
async function handleBootstrapFirstRunStarted(event: PlatformEvent): Promise<void> {
  const { tenantId, __payload } = event;
  if (!tenantId) return;
  await insertNotification(tenantId, {
...
function wrapHandler(name: string, fn: (event: PlatformEvent) => Promise<void>): EventHandler {
  return async (payload: Record<string, unknown>) => {
    const event = payload as any as PlatformEvent;
    try {
      await fn(event);
...
export function registerNotificationEventSubscribers(): void {
  for (const [eventName, handler] of handlers) {
    eventBus.subscribe(eventName, `notification:${eventName}`, async (event: PlatformEvent) => {
      await handler(event as any as Record<string, unknown>);
    });
  }
...
try {
  eventBus.subscribe('inbox.broadcast.create' as any, 'notification:inbox.broadcast', handleInboxBroadcast);
  eventBus.subscribe('packs.auto_update' as any, 'notification:packs.update', handlePackInstallResult);
  eventBus.subscribe('integrations.webhooks' as any, 'notification:integrations.webhook', handleIntegrationSyncFailure);
} catch { /* pass */ }
