...
import { pushToUser } from "../../../platform/dos/events/websocket.service";
  ...
  for (const notif of resolved) {
    try {
      ...
      if (notif.channel === 'slack') {
        try {
          const { resolveSlackConfig } = await import('../../integrations/services/integration-config-resolver.service');
          const slackCfg = await resolveSlackConfig(tenantId);
          if (slackCfg?.webhookUrl) {
            const { sendSlackNotification } = await import('../../../connectors/slack-adapter');
      ...
      if (notif.channel === 'teams') {
        try {
          const { resolveTeamsConfig } = await import('../../integrations/services/integration-config-resolver.service');
          const teamsCfg = await resolveTeamsConfig(tenantId);
          if (teamsCfg?.webhookUrl) {
            const { sendTeamsNotification } = await import('../../../connectors/teams-adapter');
...
export async function createNotification(
  ...
  try {
    pushToUser(tenantId, data.userId, {
      type: 'notification',
      data: notification,
...
export async function getNotifications(
  ...
  return result.rows as any as Notification[];
}
