...
export function findTopByVolume<T extends { total: number; [key: string]: unknown }>(
  items: T[],
  labelKey: keyof T
): string | null {
...
export async function getDeliveryMetrics(
  ...
  try {
    ...
    return result.rows.map((r: any) => {
      ...
      const delivered = parseInt(r.total_delivered, 10);
      return {
        channel: r.channel,
        totalSent: sent,
        ...
        averageDeliveryMs: r.avg_delivery_ms ? Math.round(parseFloat(r.avg_delivery_ms)) : null,
      };
    });
  } catch { return []; }
}
...
export async function getModuleNotificationCounts(
  ...
  try {
    ...
    return result.rows.map((r: any) => {
      ...
      const read = parseInt(r.read_count, 10);
      return {
