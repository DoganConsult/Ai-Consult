...
beforeAll(async () => {
  const mod = await import('./notification.module');
  manifest = mod.NOTIFICATION_MANIFEST ?? mod.default?.manifest;
});

describe('Notification Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('notification'); });
