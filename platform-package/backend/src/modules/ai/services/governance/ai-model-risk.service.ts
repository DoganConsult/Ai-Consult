...
export interface ModelLifecycleState {
  lifecycle_id: string;
  model_version_id: string;
  current_state: 'development' | 'validation' | 'staging' | 'production' | 'deprecated' | 'archived';
...
export interface ModelRiskAssessment {
  ...
  model_version_id: string;
  system_id: string;
  assessment_type: 'automated' | 'manual' | 'hybrid' | 'external_audit';
 ...
 */
export async function calculateModelRiskScore(
  tenantId: string,
  ...
  if (modelData.training_data_summary) {
    ...
    if (dataSummary.contains_pii) dataRisk += 0.3;
    if (dataSummary.contains_sensitive_data) dataRisk += 0.2;
    if (dataSummary.bias_detected) dataRisk += 0.2;
    if (!dataSummary.quality_metrics) dataRisk += 0.1;
    if (dataSummary.data_volume && dataSummary.data_volume < 1000) dataRisk += 0.2;
  }
  ...
  if (modelData.performance_metrics) {
    ...
    if (perf.accuracy && perf.accuracy < 0.8) modelRisk += 0.3;
    if (perf.precision && perf.precision < 0.75) modelRisk += 0.2;
    if (perf.recall && perf.recall < 0.75) modelRisk += 0.2;
    if (!perf.robustness_tested) modelRisk += 0.2;
    if (perf.adversarial_vulnerability) modelRisk += 0.1;
  }
  ...
  const result = await safeQuery(
    `INSERT INTO "${schema}".ai_model_risk_scores 
     ...
     RETURNING *`,
    [modelVersionId, systemId, dataRisk, modelRisk, operationalRisk, complianceRisk, compositeRisk, JSON.stringify(riskFactors)]
  );
  ...
  try {
    ...
    if (typeof (err as Error & Record<string, unknown>)?.message === 'string') {
      logger.warn('[ai-model-risk] syncModelRiskToEnterpriseRegister failed:', (err as Error).message);
    }
  }
  return row;
}
 ...
 */
export async function syncModelRiskToEnterpriseRegister(
  tenantId: string,
  modelVersionId: string,
  systemId: string,
  compositeRiskScore: number,
  scoreRow?: { score_id?: string; risk_factors?: Record<string, any> }
): Promise<void> {
  const schema = tenantSchema(tenantId);
  ...
  const existing = await safeQuery(
    `SELECT risk_id, likelihood, impact, title FROM "${schema}".risks
 ...
 */
export async function getModelRiskScores(
  tenantId: string,
  modelVersionId?: string,
  systemId?: string
): Promise<ModelRiskScore[]> {
...
export async function transitionModelLifecycle(
  ...
  modelVersionId: string,
  systemId: string,
  newState: 'development' | 'validation' | 'staging' | 'production' | 'deprecated' | 'archived',
  ...
  const currentResult = await safeQuery(
    `SELECT current_state FROM "${schema}".ai_model_lifecycle 
  ...
  const result = await safeQuery(
    `INSERT INTO "${schema}".ai_model_lifecycle 
     ...
     RETURNING *`,
    [modelVersionId, systemId, newState, previousState, transitionReason, transitionedBy, requiresApproval, requiresApproval ? 'pending' : 'not_required']
  );
 ...
 */
export async function approveLifecycleTransition(
  tenantId: string,
  lifecycleId: string,
  approvedBy: string,
  ...
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `UPDATE "${schema}".ai_model_lifecycle 
    ...
    [approvedBy, approvalNotes || null, lifecycleId]
  );
 ...
 */
export async function createModelRiskAssessment(
  tenantId: string,
  modelVersionId: string,
  systemId: string,
  assessment: {
  ...
  if (assessment.risk_findings && assessment.risk_findings.length > 0) {
    const criticalFindings = assessment.risk_findings.filter((f: GenericRow) => f.severity === 'critical').length;
    const highFindings = assessment.risk_findings.filter((f: GenericRow) => f.severity === 'high').length;
    if (criticalFindings > 0) riskLevel = 'critical';
    else if (highFindings > 2) riskLevel = 'high';
    else if (highFindings > 0) riskLevel = 'medium';
    else riskLevel = 'low';
  }

  const result = await safeQuery(
    `INSERT INTO "${schema}".ai_model_risk_assessments 
 ...
 */
export async function getModelRiskAssessments(
  tenantId: string,
  modelVersionId?: string,
  systemId?: string
): Promise<ModelRiskAssessment[]> {
 ...
 */
export async function getModelsRequiringAssessment(tenantId: string): Promise<any[]> {
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT DISTINCT m.model_version_id, m.asset_id, m.system_id, s.system_name,
