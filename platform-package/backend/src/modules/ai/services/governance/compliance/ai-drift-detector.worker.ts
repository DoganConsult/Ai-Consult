...
function isCanonicalFormat(code: string): boolean { return /^[a-z][a-z0-9_-]*\.[a-z][a-z0-9_]*\.[a-z][a-z0-9_]*$/.test(code); }
...
export interface DriftItem {
  category: 'permission_format' | 'phantom_preset' | 'missing_role_pack' | 'orphan_module' | 'unguarded_route' | 'stale_cache' | 'sod_gap';
...
export async function runDriftDetection(tenantId: string): Promise<DriftReport> {
  ...
  try {
    await safeQuery(
      ...
      [JSON.stringify({ summary, driftCount: drifts.length }), summary.critical > 0 ? 'critical' : summary.high > 0 ? 'high' : 'info'],
    );
  } catch (err) {
  ...
  try {
    await safeQuery(
      ...
      [JSON.stringify(result), summary.critical > 0 ? 'critical' : summary.high > 0 ? 'high' : 'info'],
    );
  } catch (err) {
...
export async function getLastDriftReport(tenantId: string): Promise<DriftReport | null> {
  try {
    const schema = tenantSchema(tenantId);
...
async function detectPermissionFormatDrift(schema: string, drifts: DriftItem[]): Promise<void> {
  try {
    const { rows } = await safeQuery(
      `SELECT permission_code, module_code FROM "${schema}".module_permissions WHERE is_active = true`,
    );
    for (const r of rows) {
...
async function detectMissingRolePacks(schema: string, drifts: DriftItem[]): Promise<void> {
  try {
    const { rows } = await safeQuery(
      `SELECT DISTINCT module_code FROM "${schema}".module_role_definitions WHERE is_active = true`,
    );
    const modulesWithRoles = new Set(rows.map((r: GenericRow) => r.module_code));
...
async function detectPhantomDashboardPresets(schema: string, drifts: DriftItem[]): Promise<void> {
  try {
    const { rows: activationRows } = await safeQuery(
      `SELECT module_code, metadata FROM "${schema}".module_activation_rules WHERE is_active = true AND metadata->>'dashboardPresets' IS NOT NULL`,
    );
    const { rows: dashboardRows } = await safeQuery(
      `SELECT dashboard_code FROM "${schema}".dashboard_layouts WHERE is_active = true`,
    );
    const existingPresets = new Set(dashboardRows.map((r: GenericRow) => r.dashboard_code));

    for (const r of activationRows) {
      const presets: string[] = r.metadata?.dashboardPresets || [];
...
async function detectSoDGaps(schema: string, drifts: DriftItem[]): Promise<void> {
  try {
    const { rows: modulesWithActions } = await safeQuery(
      `SELECT DISTINCT module_code FROM "${schema}".module_actions WHERE is_active = true AND sod_sensitive = true`,
    );
    const { rows: modulesWithSod } = await safeQuery(
      `SELECT DISTINCT module_code FROM "${schema}".module_sod_rules WHERE is_active = true`,
    );
    const sodModules = new Set(modulesWithSod.map((r: GenericRow) => r.module_code));
...
async function detectOrphanModules(schema: string, drifts: DriftItem[]): Promise<void> {
  try {
    const { rows: activatedModules } = await safeQuery(
      `SELECT module_code FROM "${schema}".module_activation_rules WHERE is_active = true`,
    );
    const activatedSet = new Set(activatedModules.map((r: GenericRow) => r.module_code));
...
async function detectUnguardedRoutes(schema: string, drifts: DriftItem[]): Promise<void> {
  try {
    const { rows } = await safeQuery(
      `SELECT module_code, path, method FROM "${schema}".route_catalog WHERE is_active = true AND (required_permissions IS NULL OR required_permissions = '[]'::jsonb)`,
    );
    for (const r of rows) {
      drifts.push({
...
async function detectStaleCache(schema: string, drifts: DriftItem[]): Promise<void> {
  try {
    const { rows } = await safeQuery(
      `SELECT table_name, MAX(created_at) AS last_change
       FROM "${schema}".rbac_config_audit
       WHERE created_at > NOW() - INTERVAL '24 hours'
       GROUP BY table_name`,
    );
    const { rows: lastDrift } = await safeQuery(
      `SELECT MAX(created_at) AS last_run FROM "${schema}".agrc_event_log WHERE event_type = 'rbac_drift_result'`,
    );
