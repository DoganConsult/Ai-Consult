...
export async function generatePermissionRecommendations(tenantId: string): Promise<RecommendationReport> {
  ...
  try {
    ...
    const { rows } = await safeQuery(
      `SELECT user_id, permission_code, COUNT(*) AS denied_count
       ...
       ORDER BY COUNT(*) DESC
       LIMIT 100`,
      [safeLookback, MIN_DENIED_COUNT],
    );

    totalDenied = rows.reduce((sum: number, r: any) => sum + Number(r.denied_count), 0);
...
export async function getLastRecommendations(tenantId: string): Promise<RecommendationReport | null> {
  try {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(
    ...
    if (rows.length > 0) {
      return (typeof rows[0].payload === 'string' ? JSON.parse(rows[0].payload) : rows[0].payload) as RecommendationReport;
    }
  } catch (err) {
