...
import { CLAUDE_MODEL } from '../../../../config/app/claude-client';
...
import {
  ...
  callClaude, callAzureOpenAI, callFreeProvider as callFree, callOllama, callOpenAIApi,
} from './llm-providers';
...
function loadConfig(): LLMConfig {
  ...
  if (existsSync(configPath)) {
    ...
    return {
      ...
      ollama: raw.ollama
        ...
        : undefined,
      openai: raw.openai
        ? { endpoint: raw.openai.endpoint || process.env.OPENAI_BASE_URL || 'http://127.0.0.1:1234/v1', apiKey: raw.openai.apiKey || process.env.OPENAI_API_KEY || 'lm-studio', model: raw.openai.model || process.env.OPENAI_MODEL || 'local-model' }
        : (process.env.OPENAI_BASE_URL ? { endpoint: process.env.OPENAI_BASE_URL, apiKey: process.env.OPENAI_API_KEY || 'lm-studio', model: process.env.OPENAI_MODEL || 'local-model' } : undefined),
  ...
  return {
    ...
    openai: process.env.OPENAI_BASE_URL
      ? { endpoint: process.env.OPENAI_BASE_URL, apiKey: process.env.OPENAI_API_KEY || 'lm-studio', model: process.env.OPENAI_MODEL || 'local-model' }
      : undefined,
    freeProviders: Object.keys(freeProviders).length > 0 ? freeProviders : undefined,
...
export async function chatCompletion(
  ...
  // 5. Try OpenAI / LM Studio (advanced local)
  if ((provider === "openai" || provider === "lmstudio" || provider === "auto") && config.openai?.endpoint) {
    try {
      const result = await callOpenAIApi(config.openai, messages, config.maxTokens);
      return { ...result, latencyMs: Date.now() - start };
    } catch (err: unknown) {
      logger.warn(`[LLM] OpenAI/LMStudio failed (agent ${agentId || "?"}): ${toErrorMessage(err)}`);
    }
  }
...
export async function enhancedChatCompletion(
  ...
  const traceId = tenantId ? (() => { try { const { createTraceId } = require('../llm/llm-trace.service'); return createTraceId(); } catch { return undefined; } })() : undefined;
  const start = Date.now();
...
export async function agentChat(
  ...
  if (opts?.tenantId) {
    ...
    try {
      const { getLanguageAwarePrompt } = await import('../../../../ai/copilot/services/bilingual-prompt.service');
