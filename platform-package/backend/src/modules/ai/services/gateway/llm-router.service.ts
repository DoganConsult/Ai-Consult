...
const TIER_MODEL_MAP: Record<ComplexityTier, { provider: string; model: string }> = {
...
let _configCache = new Map<string, { config: AgentModelConfig; ts: number }>();
let _performanceCache = new Map<string, { metrics: ModelPerformanceMetrics; ts: number }>();
const CACHE_TTL = 60_000;
const PERFORMANCE_CACHE_TTL = 300_000; // 5 minutes

export async function getAgentModelConfig(tenantId: string, agentId: string): Promise<AgentModelConfig | null> {
  ...
  try {
    const result = await safeQuery(
      `SELECT * FROM "${schema}".agent_model_config WHERE agent_id = $1`,
      [agentId],
    );
    if (result.rows.length === 0) return null;
    const config = getFirstRow(result) as AgentModelConfig;
...
export async function upsertAgentModelConfig(
  tenantId: string,
  agentId: string,
  updates: Partial<Omit<AgentModelConfig, 'agent_id'>>,
): Promise<boolean> {
  const schema = tenantSchema(tenantId);
  try {
    await safeQuery(
      `INSERT INTO "${schema}".agent_model_config (agent_id, preferred_model, preferred_provider, max_tokens, temperature, complexity_tier, fallback_model, fallback_provider, enabled, updated_at)
...
export function estimateComplexity(taskType: string, inputTokenEstimate: number): ComplexityTier {
  const baseTier = _getRegistryComplexity(taskType) ?? 'medium';
  if (inputTokenEstimate > 6000 && baseTier === 'medium') return 'high';
  if (inputTokenEstimate > 10000) return 'critical';
  return baseTier;
}
...
export async function getModelPerformanceMetrics(
  ...
  try {
    const result = await safeQuery(
      `SELECT provider, model, avg_latency_ms, success_rate, 
       ...
       LIMIT 1`,
      [tenantId, agentId, provider, model, complexityTier, taskType || null],
    );
 ...
 */
export async function updateModelPerformanceCache(
  tenantId: string,
  agentId: string,
  provider: string,
  model: string,
  ...
  try {
    ...
    const existing = await safeQuery(
      `SELECT cache_id, avg_latency_ms, success_rate, sample_count,
    ...
    if (existing.rows.length > 0) {
      ...
      await safeQuery(
        `UPDATE "${schema}".agent_model_performance_cache
      ...
      // Create new
      await safeQuery(
        `INSERT INTO "${schema}".agent_model_performance_cache
 ...
 */
async function selectBestModelFromCache(
  tenantId: string,
  agentId: string,
  complexityTier: ComplexityTier,
  taskType?: string,
  ...
  try {
    ...
    const result = await safeQuery(
      `SELECT provider, model, avg_latency_ms, success_rate, sample_count
       FROM "${schema}".agent_model_performance_cache
       WHERE tenant_id = $1 AND agent_id = $2 
       ...
       ORDER BY 
         ...
         avg_latency_ms ASC
       LIMIT 1`,
      [tenantId, agentId, complexityTier, taskType || null],
    );
...
export async function resolveModelForAgent(
  ...
  let effectiveTier = config.complexity_tier as ComplexityTier;
  if (taskType && inputTokenEstimate) {
    const dynamicTier = estimateComplexity(taskType, inputTokenEstimate);
    const tierOrder: ComplexityTier[] = ['low', 'medium', 'high', 'critical'];
  ...
  if ((provider === 'auto' || model === 'auto') && tenantId && agentId) {
    ...
    const bestModel = await selectBestModelFromCache(
      tenantId,
      agentId,
      effectiveTier,
      taskType,
    ...
    if (bestModel) {
      if (provider === 'auto') provider = bestModel.provider;
      if (model === 'auto') model = bestModel.model;
    } else {
      // Fallback to tier defaults
      if (provider === 'auto') provider = tierConfig.provider;
      if (model === 'auto') model = tierConfig.model;
    }
...
export async function getAllAgentModelConfigs(tenantId: string): Promise<AgentModelConfig[]> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(`SELECT * FROM "${schema}".agent_model_config ORDER BY agent_id`);
    return result.rows as AgentModelConfig[];
  } catch {
    return [];
  }
}
 ...
 */
export function resolveModelForAgentLegacy(
  config: AgentModelConfig | null,
  taskType?: string,
  inputTokenEstimate?: number,
