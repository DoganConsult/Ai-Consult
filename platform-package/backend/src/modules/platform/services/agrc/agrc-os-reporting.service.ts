...
// ============================================
// Shahin GRC — AGRC-OS Reporting Service
...
import { getScheduledReports } from '../../../reporting/services/report/report.service';
import { generateExecutiveSnapshot, generateComplianceReport } from '../../../reporting/services/report/report.service';
...
export async function getReportSchedulesWithStatus(
  ...
  try {
    ...
    if (logResult.rows[0]) lastEscalationAt = logResult.rows[0].created_at?.toISOString?.() ?? logResult.rows[0].created_at;
  } catch {
...
export async function escalateOverdueReports(
  ...
  const { schedules, __overdueCount } = await getReportSchedulesWithStatus(tenantId, escalationGraceHours);
...
export async function exportLiveReport(
  ...
  if (!frameworkId) {
    try {
      ...
      const raw = fwResult.rows[0]?.framework_id;
