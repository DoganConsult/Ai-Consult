// ============================================
// Shahin GRC — AGRC-OS Dashboard Data Service
...
export interface ComplianceScore {
  overall: number;
  byFramework: { frameworkId: string; frameworkName: string; score: number; controlCount: number; effectiveCount: number }[];
  trend: { date: string; score: number }[];
  controlBreakdown: { status: string; count: number }[];
}
...
export interface ControlHealth {
  ...
  notTested: number;
  stale: number;
  byFramework: { frameworkId: string; frameworkName: string; total: number; effective: number }[];
}
...
export interface EvidenceCoverage {
  ...
  overdueTasks: number;
  byFramework: { frameworkCode: string; total: number; covered: number; percent: number }[];
}
...
export async function getRiskHeatmapData(tenantId: string): Promise<RiskHeatmapCell[]> {
...
export async function getComplianceScoreData(tenantId: string): Promise<ComplianceScore> {
  const s = tenantSchema(tenantId);
  ...
  const overall = parseFloat(overallResult.rows[0]?.overall_score) || 0;
...
export async function getMaturityRadarData(tenantId: string): Promise<MaturityDimension[]> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT
...
export async function getFindingsBarData(tenantId: string): Promise<FindingCategory[]> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT
...
export async function getEvidenceDonutData(tenantId: string): Promise<EvidenceCoverage> {
  const s = tenantSchema(tenantId);

  // Total controls and those with evidence tasks
  const controlsResult = await safeQuery(
    `SELECT COUNT(*)::int AS total FROM "${s}".controls`,
  );
  const totalControls = parseInt(controlsResult.rows[0]?.total) || 0;
  ...
  const coveredControls = parseInt(coveredResult.rows[0]?.covered) || 0;
...
export async function getVendorBubbleData(tenantId: string): Promise<VendorBubble[]> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT
       v.vendor_id,
...
export async function getTopRisksData(tenantId: string): Promise<TopRisk[]> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT
       r.risk_id,
...
export async function getTrendLineData(tenantId: string, days: number = 30): Promise<TrendSeries[]> {
  const s = tenantSchema(tenantId);
...
export async function getControlHealthData(tenantId: string): Promise<ControlHealth> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT
       COUNT(*)::int AS total,
...
export async function getIncidentStatsData(tenantId: string): Promise<IncidentStats> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT
       COUNT(*)::int AS total,
  ...
  const monthlyResult = await safeQuery(
    `SELECT
...
export async function getPolicyComplianceData(tenantId: string): Promise<PolicyCompliance> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT
       COUNT(*)::int AS total,
...
export async function getSystemOverviewStats(tenantId: string): Promise<SystemOverview> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT
