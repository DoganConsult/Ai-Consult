...
// ============================================
// Shahin GRC — AGRC-OS UI config service
// Drawer, dashboard layouts, workspace profile,
// action items for drawer. Tenant-scoped only.
// ============================================

import { safeQuery, tenantSchema } from '../../../../config/database';
import { getLatestAnswers } from '../../../onboarding/services/journey/workspace-lifecycle.service';
import { getBaselineDashboardLayoutPayload } from '../../../../data/dashboard-layout-baselines';
...
export async function getDrawerTemplate(
  ...
  try {
    ...
    if (result.rows.length === 0) {
      ...
      return mapRowToDrawerTemplate(fallback.rows[0]);
    }
    return mapRowToDrawerTemplate(result.rows[0]);
  } catch {
    return null;
  }
}

function mapRowToDrawerTemplate(r: any): DrawerTemplate {
...
export async function getDashboardLayout(
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return getBaselineDashboardLayoutPayload(dashboardCode);
    }
    const r = result.rows[0];
