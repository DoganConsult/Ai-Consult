...
export async function getFoundationUsers(tenantId: string): Promise<any> {
...
export async function getFoundationUserDetail(tenantId: string, userId: string): Promise<any> {
  const res = await safeQuery(
    `SELECT u.user_id, u.email, u.display_name, u.role, u.department_id, u.status
     FROM public.users u
...
export async function getFoundationTeams(tenantId: string): Promise<any> {
...
export async function getFoundationDepartments(tenantId: string): Promise<any> {
  const { schema } = ctx(tenantId);
  const res = await safeQuery(
...
export async function getFoundationBusinessUnits(tenantId: string): Promise<any> {
  const { schema } = ctx(tenantId);
  const res = await safeQuery(
