...
import { pct, computeMaturity, ctx, cadenceToDays } from "../misc/compliance.utils";
...
export async function getComplianceOverview(tenantId: string, options?: {
  ...
  const openFindings = findings.filter((f: GenericRow) => !['closed', 'resolved'].includes(f.status)) as string;
  const criticalGaps = openFindings.filter((f: GenericRow) => f.severity === 'critical').length;

  // Overdue remediation
  const now = new Date();
  const overdueRem = remTasks.filter((t: GenericRow) => t.due_date && new Date(t.due_date) < now && !['completed', 'closed'].includes(t.status)) as string;
  ...
  const freshCount = evidence.filter((e: GenericRow) => {
    ...
    if (e.freshness_days && e.freshness_days > 0) {
      ...
      expectedWindowDays = cadenceToDays(e.expected_cadence) as string;
    } else {
  ...
  let domainSummaries: Array<Record<string, any>> = [];
  if (!light && activeIds.length > 0) {
    ...
    domainSummaries.sort((a: GenericRow, b: GenericRow) => Number(a.score) - Number(b.score));
    domainSummaries = domainSummaries.slice(0, 20);
  }

  // Priority issues
  const priorityIssues: Array<Record<string, any>> = [];
...
export async function getAuditReadiness(tenantId: string, options?: { scope?: ComplianceScope; userId?: string }): Promise<any> {
  const { schema } = ctx(tenantId);
  const scopeMy = options?.scope === "my" && options?.userId;
  const ctrlWhere = scopeMy ? `deleted_at IS NULL AND owner = $1` : `deleted_at IS NULL`;
