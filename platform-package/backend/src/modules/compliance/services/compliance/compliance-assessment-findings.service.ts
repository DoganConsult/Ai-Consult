...
import { emptyResult, safeQuery, safeQueryWithClient, withTransaction } from "../../../../config/database";
import { v4 as uuid } from "uuid";
import { pct, computeMaturity, ctx } from "../misc/compliance.utils";
import type { ComplianceScope } from "../misc/compliance.utils";
import { eventBus } from "../../../platform/services/event/event-bus.service";
import { getComplianceSettings } from "./compliance-settings.service";
...
import { swallow, swallowDefault, EC } from '../../../../utils/resilient-catch';
...
export async function assessFramework(tenantId: string, frameworkCode: string, userId?: string): Promise<any> {
...
export async function getCoverageMatrix(tenantId: string, frameworkId: string): Promise<any> {
  const { schema } = ctx(tenantId);
...
export async function getAssessmentHistory(tenantId: string): Promise<any> {
  const { schema } = ctx(tenantId);
...
export async function createFinding(tenantId: string, data: {
  ...
  eventBus.publish({ eventType: 'compliance.finding_raised', tenantId, sourceService: 'ComplianceWorkspaceService', severity: 'info', payload: { findingId, title: data.title, severity: data.severity } });
...
export async function updateFinding(tenantId: string, findingId: string, data: {
  ...
  if (row && data.status === 'closed') {
    eventBus.publish({ eventType: 'compliance.finding_closed', tenantId, sourceService: 'ComplianceWorkspaceService', severity: 'info', payload: { findingId, status: 'closed' } });
  }
  return row;
}
...
Provide prioritized, actionable remediation steps.`;
    ...
    const aiResponse = await gatewayJSON<AISuggestionResponse>({
      tenantId,
      systemPrompt,
      userMessage,
      maxTokens: 1000,
...
export async function bulkUpdateFindingStatus(
  ...
  if (updated > 0) {
    ...
    }), { tenantId, operation: 'eventBus:compliance.finding_bulk_updated' });
  }
  return { updated };
}
...
export async function bulkAssignControls(
  ...
  const res = await safeQuery(
    `UPDATE "${schema}".controls
     ...
     WHERE control_id = ANY($2::uuid[]) AND deleted_at IS NULL
  ...
  if (updated > 0) {
    swallow(EC.EVENT_BUS, eventBus.publish({
      eventType: 'compliance.control_bulk_assigned',
      tenantId,
      sourceService: 'ComplianceWorkspaceService',
      severity: 'info',
      payload: { controlIds, ownerId, updated }
    }), { tenantId, operation: 'eventBus:compliance.control_bulk_assigned' });
  }
  return { updated };
}
...
export async function assignReviewer(tenantId: string, assessmentId: string, reviewerId: string): Promise<any> {
  const { schema } = ctx(tenantId);
  const res = await safeQuery(
...
export async function submitForReview(tenantId: string, assessmentId: string): Promise<any> {
  const { schema } = ctx(tenantId);
  const res = await safeQuery(
...
export async function approveAssessment(tenantId: string, assessmentId: string, notes?: string): Promise<any> {
  const { schema } = ctx(tenantId);
  const res = await safeQuery(
...
export async function rejectAssessment(tenantId: string, assessmentId: string, notes: string): Promise<any> {
  const { schema } = ctx(tenantId);
  const res = await safeQuery(
...
export async function getControlMonitoring(
  ...
  for (const c of controls) {
    ...
    const daysSinceTest = c.last_tested_at
      ...
      : null;
    const evidenceExpiryNext = expiryByControl.get(c.control_id) || null as string;
...
export async function getSavingsMetrics(tenantId: string): Promise<any> {
  const { schema } = ctx(tenantId);
...
export async function getComplianceByOrgUnit(tenantId: string, groupBy: 'department' | 'business_unit' = 'department'): Promise<any> {
  const { schema } = ctx(tenantId);
