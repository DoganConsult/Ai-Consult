...
export async function getComplianceCalendar(tenantId: string, from: string, to: string): Promise<any> {
  const { schema } = ctx(tenantId);
  const events: any[] = [];

  const [gapsRes, assessRes, ctrlRes, oblRes] = await Promise.all([
    safeQuery(
      `SELECT finding_id, title, severity, due_date, status
       FROM "${schema}".findings
       WHERE deleted_at IS NULL AND due_date IS NOT NULL
         AND due_date >= $1::date AND due_date <= $2::date
       ORDER BY due_date`, [from, to]),
    safeQuery(
  ...
  for (const g of gapsRes.rows) {
    events.push({
      entityId: g.finding_id,
      title: g.title,
      date: g.due_date,
      type: 'gap_due',
