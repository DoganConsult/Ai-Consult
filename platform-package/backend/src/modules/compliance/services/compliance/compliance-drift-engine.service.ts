...
export async function runDriftScan(tenantId: string, category?: string): Promise<{ eventsDetected: number; results: any[] }> {
  const schema = tenantSchema(tenantId);
  ...
  for (const rule of rules) {
    try {
      ...
      if (drifted) {
        ...
        emitEvent({ tenantId, userId: SYSTEM_JOB_ACTOR, module: 'compliance', event: 'drift_detected', entityType: 'compliance_drift_event', entityId: eventId, data: { ruleCode: rule.rule_code, severity: rule.severity, driftedCount: res.rows.length } }).catch(() => {});
      }
...
export async function listDriftEvents(tenantId: string, filters?: { status?: string; severity?: string; ruleId?: string }): Promise<any[]> {
  const schema = tenantSchema(tenantId);
