// @ts-nocheck
// ============================================================================
// Shahin GRC — Why-Compliant Explanation Engine (F25)
//
...
import { getLatestAssertion, getAssertionHistory } from "./compliance-assertion.service";
import type { ComplianceAssertion, ReasoningStep } from "./compliance-assertion.service";
...
export interface AuditDefensePackage {
  ...
  assertionHistory: Record<string, any>[];
...
export async function generateWhyCompliant(
  ...
  const evidenceCited = await fetchEvidenceCitations(schema, controlId);
...
export async function generateAuditDefensePackage(
  ...
  const evidenceChain = await fetchEvidenceCitations(schema, controlId);
  ...
  eventBus.publish("compliance.audit_defense_generated", {
    tenantId,
    controlId,
 ...
 */
async function fetchEvidenceCitations(schema: string, controlId: string): Promise<EvidenceCitation[]> {
  const res = await safeQuery(
    `SELECT e.id, e.title, e.status, e.submitted_at, e.expiry_date,
            es.composite_score
     FROM "${schema}".evidence e
     LEFT JOIN "${schema}".evidence_scores es ON es.evidence_id = e.id
     WHERE e.control_id = $1 AND e.deleted_at IS NULL
  ...
  return res.rows.map((r: GenericRow) => {
    ...
    return {
      ...
      title: r.title || "",
      status: statusEn,
      qualityScore: r.composite_score ?? null,
      submittedAt: r.submitted_at || null,
      expiryDate: r.expiry_date || null,
      descriptionEn: `Evidence "${r.title || r.id}" — status: ${statusEn}, quality: ${r.composite_score ?? "not scored"}`,
      descriptionAr: `الدليل "${r.title || r.id}" — الحالة: ${statusAr[String(statusEn)] || statusEn}، الجودة: ${r.composite_score ?? "غير مقيّم"}`,
    };
  });
}
...
async function fetchGaps(schema: string, controlId: string): Promise<GapItem[]> {
  ...
  try {
    ...
    return res.rows.map((r: GenericRow) => {
      const __severityAr: Record<string, string> = {
        critical: "حرج",
        high: "عالي",
        medium: "متوسط",
        low: "منخفض",
