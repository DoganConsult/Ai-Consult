...
import { safeQuery, safeQueryWithClient, withTransaction } from "../../../../config/database";
import { v4 as uuid } from "uuid";
import { pct, ctx } from "../misc/compliance.utils";
...
import { swallowNull, EC  } from '../../../../utils/resilient-catch';
...
export async function getGapsRegister(
  ...
  const items = res.rows.map((g: GenericRow) => ({
    ...
    remediation: remMap[String(g.gap_id)] || null,
  }));

  return { items, total };
}

export async function getGapDetail(tenantId: string, gapId: string): Promise<any> {
  const { schema } = ctx(tenantId);
...
export async function updateGap(tenantId: string, gapId: string, data: { status?: string; owner?: string }): Promise<any> {
  const { schema } = ctx(tenantId);
  const sets: string[] = [];
  const params: unknown[] = [];
  let idx = 1;
  ...
  if (row && data.status && ['closed', 'remediated'].includes(data.status)) {
    eventBus.publish({ eventType: 'compliance.gap_closed', tenantId, sourceService: 'ComplianceWorkspaceService', severity: 'info', payload: { gapId, status: data.status } });
  }
  return row;
}
...
export async function createGapRemediationTask(tenantId: string, gapId: string, data: {
  ...
  eventBus.publish({ eventType: 'compliance.gap_remediation_started', tenantId, sourceService: 'ComplianceWorkspaceService', severity: 'info', payload: { gapId, taskId } });
  return row;
}
...
export async function getComplianceRoadmap(tenantId: string): Promise<any> {
  const { __schema } = ctx(tenantId);
...
export async function generateRoadmap(tenantId: string): Promise<any> {
  const { schema } = ctx(tenantId);
...
export async function updateMilestone(_tenantId: string, milestoneId: string, data: { status?: string; notes?: string }): Promise<any> {
  const sets: string[] = [];
  const params: unknown[] = [];
  let idx = 1;
  if (data.status) { sets.push(`status = $${idx}`); params.push(data.status); idx++; }
...
export async function updateRoadmapTask(_tenantId: string, taskId: string, data: {
