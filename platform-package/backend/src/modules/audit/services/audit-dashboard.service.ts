...
export class AuditDashboardService {
  ...
  async getDashboardSummary(tenantId: string): Promise<AuditDashboardSummary> {
    ...
    logger.info('[audit] fetching dashboard summary', { tenantId });
  ...
  async getAuditTrend(tenantId: string, days: number): Promise<AuditTrendDataPoint[]> {
    ...
    return rows.map((r: Record<string, any>) => ({
      date: r.date,
      auditsStarted: parseInt(r.audits_started ?? '0', 10),
      auditsCompleted: parseInt(r.audits_completed ?? '0', 10),
      findingsRaised: parseInt(r.findings_raised ?? '0', 10),
    }));
  }
}
