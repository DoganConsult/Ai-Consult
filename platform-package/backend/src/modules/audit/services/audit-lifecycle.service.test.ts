...
} from './audit-lifecycle.service';
import { safeQuery } from '../../../config/database';

describe('Audit Lifecycle Service', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('domain queries', () => {
    it('getOpenFindings should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getOpenFindings('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getOpenFindings should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getOpenFindings('t1');
      expect(result).toEqual([]);
    });

    it('getOverdueEngagements should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getOverdueEngagements('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getOverdueEngagements should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getOverdueEngagements('t1');
      expect(result).toEqual([]);
    });
