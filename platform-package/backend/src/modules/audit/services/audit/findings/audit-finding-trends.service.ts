...
export async function getTrends(tenantId: string, startDate: string, endDate: string): Promise<any> {
...
export async function getAgingAnalysis(tenantId: string): Promise<any> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT
       CASE
...
export async function getSeverityDistribution(tenantId: string): Promise<any> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT
       severity,
...
export async function getTopRecurring(tenantId: string, limit: number = 10): Promise<any> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT
       f.finding_id,
