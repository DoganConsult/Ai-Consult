...
export async function getSlaConfig(tenantId: string): Promise<any> {
...
export async function getBreachedFindings(tenantId: string): Promise<any> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
...
export async function getSlaCompliance(tenantId: string): Promise<any> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `WITH finding_sla AS (
