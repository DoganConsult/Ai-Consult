...
import { listFiles, getFile } from '../../../../../platform/dos/storage/file-storage.service';
...
export async function generateAuditPackage(
  ...
  for (const row of evidenceRes.rows) {
    const domain = inferControlDomain(row.frameworks);
    ...
    const filename = sanitizeFilename(row.title || `evidence-${row.evidence_id}`) + '.txt';
...
async function fetchEvidenceContent(
  ...
  if (entry._filePath) {
    return getFile(entry._filePath);
  }
...
async function fetchPolicyContent(
  ...
  try {
    const files = await listFiles(tenantId, 'policy', entry._entityId);
    if (files.length > 0) {
