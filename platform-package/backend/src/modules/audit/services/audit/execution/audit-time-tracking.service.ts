...
export async function listEntries(tenantId: string, auditId: string): Promise<any> {
...
export async function getEfficiencyMetrics(tenantId: string): Promise<any> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT
...
export async function getUtilization(tenantId: string): Promise<any> {
  const s = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT
       user_id,
