...
function computeEntityHash(entity: Record<string, any>): string {
...
export async function buildAkbPackage(
  ...
  for (const e of evidenceRes.rows) {
    ...
    if (!evidenceByControl[e.control_id]) evidenceByControl[e.control_id] = [];
    evidenceByControl[e.control_id].push(e);
  }

  // ── Build Controls Detail ─────────────────────────────────
  const controls: AkbControlDetail[] = controlsRes.rows.map((c: GenericRow) => {
    const evList = evidenceByControl[String(c.control_id)] || [];
  ...
  for (const ctrl of controls) {
    for (const fw of ctrl.frameworks) {
      if (!frameworkMap[fw]) {
        const fwAssessments = assessmentsRes.rows.filter((a: unknown) => a.framework_id === fw);
        ...
        frameworkMap[fw] = {
          ...
          lastAssessmentScore: latestAssessment ? parseFloat(latestAssessment.score) || 0 : null,
        };
      }
  ...
  for (const r of risksRes.rows) {
    ...
    if (s >= 20) distribution.critical++;
    else if (s >= 12) distribution.high++;
    else if (s >= 6) distribution.medium++;
  ...
  // ── Build Statistics ──────────────────────────────────────
  const completedAssessments = assessmentsRes.rows.filter((a: unknown) => a.status === 'completed').length;
  ...
  const openHighRisks = risksRes.rows.filter((r: GenericRow) =>
    (r.risk_score >= 12 || (Number(r.likelihood) * Number(r.impact)) >= 12) && r.status !== 'mitigated'
  ).length;
  ...
  const healthScore = Math.round(
    ...
    ((100 - Math.min(100, (risksRes.rows.length > 0
      ? risksRes.rows.reduce((s: number, r: unknown) => s + (r.risk_score || 0), 0) / risksRes.rows.length * 5
      : 0))) * 0.15)
  );
  ...
  const executiveSummary = {
    healthScore,
    complianceScore,
    riskScore: Math.round(risksRes.rows.reduce((s: number, r: unknown) => s + (r.risk_score || 0), 0) / Math.max(risksRes.rows.length, 1) * 100) / 100,
  ...
  return {
    ...
    riskPosture: {
      ...
      averageScore: risksRes.rows.length > 0
        ? Math.round(risksRes.rows.reduce((s: number, r: unknown) => s + (r.risk_score || 0), 0) / risksRes.rows.length * 100) / 100
        : 0,
    },
    policies,
    remediations,
