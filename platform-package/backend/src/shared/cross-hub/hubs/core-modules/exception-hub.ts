  ...
  try {
    await safeQuery(
      ...
      [entityType, entityId, gateAction, allowed, reason,
       ...
       JSON.stringify({ ...context, _evaluationDurationMs: durationMs }), evaluatedBy || 'system'],
    );
...
export async function evaluateBatchGate(
  ...
  for (const entry of entities) {
    ...
    if (engineResults.events.length > 0) {
      ...
      await recordGateAudit(tenantId, entityType, entry.entityId, gateAction, true, result.reason, matchedRuleIds, result.matchedExceptionId, entry.facts, 'system', durationMs);
      ...
      await recordGateAudit(tenantId, entityType, entry.entityId, gateAction, false, result.reason, [], undefined, entry.facts, 'system', durationMs);
      results.set(entry.entityId, result);
    }
  }
  return results;
...
async function evaluateGate(
  ...
  if (engineResults.events.length > 0) {
    ...
    await recordGateAudit(tenantId, entityType, entityId, gateAction, true, result.reason, matchedRuleIds, result.matchedExceptionId, contextFacts, 'system', durationMs);
  ...
  await recordGateAudit(tenantId, entityType, entityId, gateAction, false, result.reason, [], undefined, contextFacts, 'system', durationMs);
...
export async function updateGatingRule(
  ...
  if (updates.conditionsJson !== undefined) {
    try {
      const testEngine = new Engine();
      testEngine.addRule({
        conditions: updates.conditionsJson as any,
