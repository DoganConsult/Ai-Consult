...
import { createAlert } from '../../modules/ai/services/governance/compliance/ai-alert.service';
import { runAgent } from '../../../../modules/ai/services/agents/core/agent-runner.service';
...
export function registerRiskHub(sub: SubFn): void {
  ...
  sub('risk.score_changed', 'xhub-risk→analytics-recompute', async (e) => {
    ...
    await safePublish({
      ...
      severity: (payload.newScore || 0) > 15 ? 'warning' : 'info',
