...
import { getPlaybookTemplateForCategory } from '../../modules/incident/services/incident/incident-playbook-templates.service';
import { createProcessTask } from '../../modules/workflow/services/tasks/process-orchestration.service';
import { createLink } from '../../../../modules/platform/services/entity-link/index';
...
export function registerIncidentHub(sub: SubFn): void {
  ...
  sub('incident.root_cause_identified', 'xhub-incident→control-effectiveness', async (e) => {
    ...
    const controlIds: string[] = payload?.control_id
      ...
      : (Array.isArray(payload?.affected_controls) ? payload.affected_controls.map((c: unknown) => String(c)) : []);
  ...
  sub('control.effectiveness_low', 'xhub-control-eff→remediation-a04-risk', async (e) => {
    ...
    try {
      const { recalculateRiskScores } = await import('../../modules/risk/services/auto-risk-scoring.service');
